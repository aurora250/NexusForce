...
void test_sql() {
    ...
    auto sql9 = sql_builder()
                        ...
                        .from_subquery("SELECT user_id, COUNT(*) as total_orders FROM orders GROUP BY user_id", "o")
                        .inner_join("users u", "u.id = o.user_id")
                        .where_gt("o.total_orders", "5")
                        .build();
    println(sql9);

    auto sql10 = sql_builder().distinct().select({"city", "country"}).from("users").order_by_asc("country").build();
    println(sql10);
