...
TEST_F(H2TlsIntegrationTest, NoAlpnReturnsEmpty) {
    ssl_stream s;
    EXPECT_TRUE(s.alpn_negotiated().empty());
...
class HttpClientLocalServerIntegration : public ::testing::Test {
protected:
    void SetUp() override {}
    void TearDown() override {}
...
TEST_F(HttpClientLocalServerIntegration, SyncRequestRoundtrip) {
    ports test_port(0u);
    {
        tcp_acceptor tmp;
    ...
    server.router().get("/hello", [](http_request&, http_response& res) {
        res.status = http_status::S2_OK;
        res.status_message = "OK";
        res.body = "local-hello";
    });
    if (!server.start()) {
    ...
    auto response = client.get("http://127.0.0.1:" + to_string(test_port.value()) + "/hello");
    EXPECT_TRUE(response.is_success());
    EXPECT_EQ(response.body, "local-hello");

    client.close();
    server.stop();
}

TEST_F(HttpClientLocalServerIntegration, RequestAsyncFuture) {
    ports test_port(0u);
    {
        tcp_acceptor tmp;
        tmp.open(ip_address::loopback());
    ...
    http_server server(test_port, server_ctx_, 1);
    server.router().get("/async", [](http_request&, http_response& res) {
        res.status = http_status::S2_OK;
        res.status_message = "OK";
        res.body = "async-body";
    });
    ...
    http_client_response response = fut.get();
    EXPECT_TRUE(response.is_success());
...
TEST_F(HttpClientLocalServerIntegration, RequestAsyncUseAwaitable) {
    ports test_port(0u);
    {
        tcp_acceptor tmp;
        tmp.open(ip_address::loopback());
    ...
    http_server server(test_port, server_ctx_, 1);
    server.router().get("/await", [](http_request&, http_response& res) {
        res.status = http_status::S2_OK;
        res.status_message = "OK";
        res.body = "awaitable-body";
    });
    if (!server.start()) {
        GTEST_SKIP() << "http_server failed to bind local port";
    }
    ...
    http_client_response response = awaiter.await_resume();
    EXPECT_TRUE(response.is_success());
    EXPECT_EQ(response.body, "awaitable-body");
