...
- **Random Number Generation** - LC, Mersenne Twister, PCG, xoroshiro, bit-width adapter and hardware noise algorithms `random_lcd` / `random_mt` / `random_pcg32` / `random_pcg64` / `random_xoroshiro128` / `random_xoroshiro256` / `bit_gen` / `secret`
- **Random Distributions** - uniform, Bernoulli, normal, exponential, log-uniform and Poisson distributions `uniform_int` / `uniform_real` / `bernoulli` / `normal` / `exponential` / `log_uniform` / `poisson`, engines satisfy the standard UniformRandomBitGenerator concept
...
Copyright notices and full license texts for the third-party code included in,
and the libraries linked into, this project are in
[THIRD_PARTY_NOTICES](THIRD_PARTY_NOTICES.md). Ship that file with any binary
distribution: the MIT License alone does not cover the attribution requirements
of the ported code and linked libraries.
