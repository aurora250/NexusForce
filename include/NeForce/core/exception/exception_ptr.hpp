...
public:
    ...
    struct ecb {
         ...
         * @brief 增加引用计数
         */
        NEFORCE_ALWAYS_INLINE void add_ref() noexcept { ref_count.fetch_add(1, memory_order_relaxed); }
        ...
        NEFORCE_ALWAYS_INLINE void release() noexcept {
            if (ref_count.load(memory_order_acquire) == 1 || ref_count.fetch_sub(1, memory_order_acq_rel) == 1) {
                delete this;
            }
        }
    };
...
public:
     ...
     */
    void swap(exception_ptr& other) noexcept { _NEFORCE swap(ecb_, other.ecb_); }
     ...
     * 检查异常指针是否非空。
     */
    explicit operator bool() const noexcept { return ecb_ != nullptr; }
     ...
     * 比较两个异常指针是否引用同一个异常对象。
     */
    bool operator==(const exception_ptr& rhs) const noexcept { return ecb_ == rhs.ecb_; }
     ...
     * @return 是否不相等
     */
    bool operator!=(const exception_ptr& rhs) const noexcept { return !(*this == rhs); }

    /**
     * @brief 与空指针比较相等
     * @return 是否为空
     */
    bool operator==(nullptr_t) const noexcept { return !static_cast<bool>(*this); }

    /**
     * @brief 空指针与异常指针比较相等
     */
    friend bool operator==(nullptr_t, const exception_ptr& ptr) noexcept { return !ptr; }
     ...
     * @brief 空指针与异常指针比较不等
     */
    friend bool operator!=(nullptr_t, const exception_ptr& ptr) noexcept { return static_cast<bool>(ptr); }
