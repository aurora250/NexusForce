#ifndef NEFORCE_CORE_NUMERIC_STATIC_NUMERIC_HPP__
#define NEFORCE_CORE_NUMERIC_STATIC_NUMERIC_HPP__
...
private:
    ...
    static_assert((a_low * b_high + b_low * a_high) * half_range <= numeric_traits<intmax_t>::max() - b_low * a_low,
...
private:
    ...
    static constexpr uintmax_t cross_sum_high =
            cross_sum / half_range + ((cross_sum < cross_product1) ? half_range : 0);         ///< 交叉和的高位部分
...
private:
    ...
    static constexpr uintmax_t rem_high_final =
            ((rem_high2 < temp1) ? ((rem_high3 >= Den) && (rem_high3 < temp1)) ? (rem_high3 + Den) : rem_high3
                                 : rem_high2) -
            temp1;
    static constexpr uintmax_t quot_high_final =
            quot_high1 - ((rem_high2 < temp1) ? ((rem_high3 >= Den) && (rem_high3 < temp1)) ? 2 : 1 : 0);
    ...
    static constexpr uintmax_t rem_low_final =
            ((rem_low2 < temp2) ? ((rem_low3 >= Den) && (rem_low3 < temp2)) ? (rem_low3 + Den) : rem_low3 : rem_low2) -
            temp2;
    static constexpr uintmax_t quot_low_final =
            quot_low1 - ((rem_low2 < temp2) ? ((rem_low3 >= Den) && (rem_low3 < temp2)) ? 2 : 1 : 0);
...
private:
    ...
    static constexpr int leading_zeros = _NEFORCE countl_zero(Den);                        ///< 除数前导零个数
    ...
    static constexpr uintmax_t num_low_high =
            (leading_zeros != 0) ? (NumLow / shift_factor2) : 0;                  ///< 被除数低位的高位部分
...
using static_select_int_t =
        typename inner::__select_int_base<static_parse_int<Digits...>::value, unsigned char, unsigned short,
                                          unsigned int, unsigned long, unsigned long long>::type;

/** @} */ // StaticCharDigit

NEFORCE_END_NAMESPACE__
#endif // NEFORCE_CORE_NUMERIC_STATIC_NUMERIC_HPP__
