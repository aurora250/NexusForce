...
class multiset : icollector<multiset<Key, Compare, Alloc>> {
    ...
    static_assert(is_same_v<rb_tree_node<Key>, typename Alloc::value_type>, "allocator type mismatch.");
...
public:
     ...
     * @return 指向最大元素的反向迭代器
     */
    NEFORCE_NODISCARD reverse_iterator rbegin() noexcept { return reverse_iterator(tree_.end()); }

    /**
     * @brief 获取反向结束迭代器
     * @return 指向最小元素之前位置的反向迭代器
     */
    NEFORCE_NODISCARD reverse_iterator rend() noexcept { return reverse_iterator(tree_.begin()); }

    /**
     * @brief 获取常量反向起始迭代器
     * @return 指向最大元素的常量反向迭代器
     */
    NEFORCE_NODISCARD const_reverse_iterator rbegin() const noexcept { return const_reverse_iterator(tree_.cend()); }

    /**
     * @brief 获取常量反向结束迭代器
     * @return 指向最小元素之前位置的常量反向迭代器
     */
    NEFORCE_NODISCARD const_reverse_iterator rend() const noexcept { return const_reverse_iterator(tree_.cbegin()); }

    /**
     * @brief 获取常量反向起始迭代器
     * @return 指向最大元素的常量反向迭代器
     */
    NEFORCE_NODISCARD const_reverse_iterator crbegin() const noexcept { return const_reverse_iterator(tree_.cend()); }

    /**
     * @brief 获取常量反向结束迭代器
     * @return 指向最小元素之前位置的常量反向迭代器
     */
    NEFORCE_NODISCARD const_reverse_iterator crend() const noexcept { return const_reverse_iterator(tree_.cbegin()); }
     ...
     * @return 指向插入元素的迭代器
     */
    iterator insert(const value_type& value) { return tree_.insert_equal(value); }
     ...
     * @return 指向插入元素的迭代器
     */
    iterator insert(value_type&& value) { return tree_.insert_equal(_NEFORCE move(value)); }
     ...
     * @return 指向插入元素的迭代器
     */
    iterator insert(iterator position, const value_type& value) { return tree_.insert_equal(position, value); }
     ...
     */
    void swap(multiset& other) noexcept(is_nothrow_swappable_v<base_type>) { tree_.swap(other.tree_); }
     ...
     */
    NEFORCE_NODISCARD bool equal_to(const multiset& rhs) const noexcept(noexcept(tree_ == rhs.tree_)) {
     ...
     * @return 按字典序比较结果
     */
    NEFORCE_NODISCARD bool less_than(const multiset& rhs) const noexcept(noexcept(tree_ < rhs.tree_)) {
        return tree_ < rhs.tree_;
    }
};
...
multiset(Iterator, Iterator, Compare = Compare(), Alloc = Alloc()) -> multiset<iter_value_t<Iterator>, Compare, Alloc>;
