#ifndef NEFORCE_CORE_CONTAINER_SET_HPP__
#define NEFORCE_CORE_CONTAINER_SET_HPP__
...
public:
     ...
     * @return 插入结果（迭代器和是否成功）
     */
    pair<iterator, bool> insert(value_type&& value) { return tree_.insert_unique(_NEFORCE move(value)); }
     ...
     * @param last 结束迭代器
     */
    template <typename Iterator> void insert(Iterator first, Iterator last) { tree_.insert_unique(first, last); }
    ...
    /**
     * @brief 交换两个set的内容
     * @param other 要交换的另一个set
     */
    void swap(set& other) noexcept(noexcept(tree_.swap(other.tree_))) { tree_.swap(other.tree_); }
...
template <typename Key, typename Alloc> set(std::initializer_list<Key>, Alloc) -> set<Key, less<Key>, Alloc>;
#endif

/** @} */ // Container

NEFORCE_END_NAMESPACE__
#endif // NEFORCE_CORE_CONTAINER_SET_HPP__
