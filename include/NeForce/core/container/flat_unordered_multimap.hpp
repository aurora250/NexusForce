#ifndef NEFORCE_CORE_CONTAINER_FLAT_UNORDERED_MULTIMAP_HPP__
#define NEFORCE_CORE_CONTAINER_FLAT_UNORDERED_MULTIMAP_HPP__

/**
 * @file flat_unordered_multimap.hpp
 * @brief 平坦无序多重映射容器
 *
 * 此文件提供了平坦无序多重映射容器的实现。
 * flat_unordered_multimap 是一种基于开放寻址哈希表的关联容器，包含键值对，允许重复键。
 ...
 * @tparam Alloc 分配器类型
 *
 * flat_unordered_multimap 是一种关联容器，存储键值对（key-value pairs），允许重复键。
 * 元素无序存储，由哈希函数将键映射到槽位。
 * 底层使用开放寻址平坦哈希表，提供优于链地址法的缓存局部性。
 *
 * @note 与 unordered_multimap 不同，本容器使用 pair<Key, T>（非 const Key）作为值类型，
 *       以适应平坦存储的元素搬移需求。
 */
...
class flat_unordered_multimap : public icollector<flat_unordered_multimap<Key, T, HashFcn, EqualKey, Alloc>> {
    static_assert(is_hash_v<HashFcn, Key>, "flat_unordered_multimap requires valid hash function.");
    static_assert(is_allocator_v<Alloc>, "Alloc type is not a standard allocator type.");
    static_assert(is_object_v<Key>, "flat_unordered_multimap only contains object types.");
...
public:
     ...
     * @param n 初始容量提示
     */
    explicit flat_unordered_multimap(const size_type n) :
    ht_(n) {}
    ...
    /**
     * @brief 拷贝构造函数
     * @param other 源 flat_unordered_multimap
     */
     ...
     * @return 自身引用
     */
    flat_unordered_multimap&
    operator=(flat_unordered_multimap&& other) noexcept(is_nothrow_move_assignable_v<base_type>) {
        ht_ = _NEFORCE move(other.ht_);
        return *this;
    }
    ...
    flat_unordered_multimap(ilist.begin(), ilist.end()) {}
    ...
    flat_unordered_multimap(ilist.begin(), ilist.end(), n) {}
    ...
    flat_unordered_multimap(ilist.begin(), ilist.end(), n, hf) {}
    ...
    flat_unordered_multimap(std::initializer_list<value_type> ilist, const size_type n, const hasher& hf,
                            const key_equal& eql) :
    flat_unordered_multimap(ilist.begin(), ilist.end(), n, hf, eql) {}
     ...
     * @param other 要交换的容器
     */
    void swap(flat_unordered_multimap& other) noexcept(is_nothrow_swappable_v<base_type>) { ht_.swap(other.ht_); }
     ...
     * @return 是否相等
     */
    NEFORCE_NODISCARD bool equal_to(const flat_unordered_multimap& rhs) const noexcept(noexcept(ht_ == rhs.ht_)) {
        return ht_ == rhs.ht_;
    }
     ...
     * @return 按字典序比较结果
     */
    NEFORCE_NODISCARD bool less_than(const flat_unordered_multimap& rhs) const noexcept(noexcept(ht_ < rhs.ht_)) {
        return ht_ < rhs.ht_;
    }
};
...
flat_unordered_multimap(Iterator, Iterator, HashFcn = HashFcn(), Compare = Compare(), Alloc = Alloc())
        -> flat_unordered_multimap<iter_map_key_t<Iterator>, iter_map_value_t<Iterator>, HashFcn, Compare, Alloc>;

template <typename Key, typename T, typename HashFcn = hash<Key>, typename Compare = equal_to<Key>,
          typename Alloc = allocator<pair<Key, T>>>
flat_unordered_multimap(std::initializer_list<pair<Key, T>>, HashFcn = HashFcn(), Compare = Compare(), Alloc = Alloc())
        -> flat_unordered_multimap<Key, T, HashFcn, Compare, Alloc>;

template <typename Iterator, typename Alloc>
flat_unordered_multimap(Iterator, Iterator, Alloc)
        -> flat_unordered_multimap<iter_map_key_t<Iterator>, iter_map_value_t<Iterator>, hash<iter_map_key_t<Iterator>>,
                                   equal_to<iter_map_key_t<Iterator>>, Alloc>;

template <typename Iterator, typename HashFcn, typename Alloc>
flat_unordered_multimap(Iterator, Iterator, HashFcn, Alloc)
        -> flat_unordered_multimap<iter_map_key_t<Iterator>, iter_map_value_t<Iterator>, HashFcn,
                                   equal_to<iter_map_key_t<Iterator>>, Alloc>;

template <typename Key, typename T, typename Alloc>
flat_unordered_multimap(std::initializer_list<pair<Key, T>>, Alloc)
        -> flat_unordered_multimap<Key, T, hash<Key>, equal_to<Key>, Alloc>;

template <typename Key, typename T, typename HashFcn, typename Alloc>
flat_unordered_multimap(std::initializer_list<pair<Key, T>>, HashFcn, Alloc)
        -> flat_unordered_multimap<Key, T, HashFcn, equal_to<Key>, Alloc>;
#endif

/** @} */ // Container

NEFORCE_END_NAMESPACE__
#endif // NEFORCE_CORE_CONTAINER_FLAT_UNORDERED_MULTIMAP_HPP__
