...
public:
    ...
    NEFORCE_NODISCARD constexpr reference dereference() const noexcept {
        ...
        NEFORCE_DEBUG_VERIFY(current_ >= container_->data() && current_ < container_->data() + Size,
    ...
    constexpr void increment() noexcept {
        ...
        NEFORCE_DEBUG_VERIFY(current_ < container_->data() + Size, "Attempting to increment out of boundary");
        ++current_;
    }
    ...
    constexpr void decrement() noexcept {
        ...
        NEFORCE_DEBUG_VERIFY(current_ > container_->data(), "Attempting to decrement out of boundary");
        --current_;
    }
    ...
    constexpr void advance(difference_type off) noexcept {
        ...
        NEFORCE_DEBUG_VERIFY(current_ + off >= container_->data() && current_ + off <= container_->data() + Size,
     ...
     * @return 当前指针
     */
    NEFORCE_NODISCARD constexpr pointer base() const noexcept { return current_; }

    /**
     * @brief 获取关联容器
     * @return 关联容器指针
     */
    NEFORCE_NODISCARD constexpr const container_type* container() const noexcept { return container_; }
};
...
public:
     ...
     * @return 指向第一个元素的迭代器
     */
    NEFORCE_NODISCARD constexpr iterator begin() noexcept { return iterator(array_, this); }

    /**
     * @brief 获取结束迭代器
     * @return 指向无效元素的迭代器
     */
    NEFORCE_NODISCARD constexpr iterator end() noexcept { return iterator(array_ + Size, this); }

    /**
     * @brief 获取常量起始迭代器
     * @return 指向第一个元素的常量迭代器
     */
    NEFORCE_NODISCARD constexpr const_iterator begin() const noexcept { return const_iterator(array_, this); }

    /**
     * @brief 获取常量结束迭代器
     * @return 指向无效元素的常量迭代器
     */
    NEFORCE_NODISCARD constexpr const_iterator end() const noexcept { return const_iterator(array_ + Size, this); }

    /**
     * @brief 获取反向起始迭代器
     * @return 指向无效元素的反向迭代器
     */
    NEFORCE_NODISCARD constexpr reverse_iterator rbegin() noexcept { return reverse_iterator(end()); }

    /**
     * @brief 获取反向结束迭代器
     * @return 指向第一个元素的反向迭代器
     */
    NEFORCE_NODISCARD constexpr reverse_iterator rend() noexcept { return reverse_iterator(begin()); }
     ...
     * @return 指向第一个元素的常量迭代器
     */
    NEFORCE_NODISCARD constexpr const_iterator cbegin() const noexcept { return const_iterator(array_, this); }

    /**
     * @brief 获取常量结束迭代器
     * @return 指向无效元素的常量迭代器
     */
    NEFORCE_NODISCARD constexpr const_iterator cend() const noexcept { return const_iterator(array_ + Size, this); }
     ...
     * @return 第一个元素的引用
     */
    NEFORCE_NODISCARD NEFORCE_ALWAYS_INLINE constexpr reference front() noexcept { return array_[0]; }
    /**
     * @brief 访问第一个常量元素
     * @return 第一个元素的常量引用
     */
    NEFORCE_NODISCARD NEFORCE_ALWAYS_INLINE constexpr const_reference front() const noexcept { return array_[0]; }
    /**
     * @brief 访问最后一个元素
     * @return 最后一个元素的引用
     */
    NEFORCE_NODISCARD NEFORCE_ALWAYS_INLINE constexpr reference back() noexcept { return array_[Size - 1]; }
    /**
     * @brief 访问最后一个常量元素
     * @return 最后一个元素的常量引用
     */
    NEFORCE_NODISCARD NEFORCE_ALWAYS_INLINE constexpr const_reference back() const noexcept { return array_[Size - 1]; }
    /**
     * @brief 获取底层数据指针
     * @return 指向数组第一个元素的指针
     */
    NEFORCE_NODISCARD NEFORCE_CONST_FUNCTION NEFORCE_ALWAYS_INLINE constexpr T* data() noexcept { return array_; }
    /**
     * @brief 获取常量底层数据指针
     ...
     * @param value 填充的值
     */
    constexpr void fill(const T& value) { _NEFORCE fill_n(array_, Size, value); }

    /**
     * @brief 交换两个数组
     * @param other 要交换的另一个数组
     */
    constexpr void swap(array& other) noexcept(is_nothrow_swappable_v<T>) { _NEFORCE swap(array_, other.array_); }
...
public:
    ...
    NEFORCE_NODISCARD NEFORCE_CONST_FUNCTION NEFORCE_ALWAYS_INLINE constexpr iterator end() noexcept {
        return iterator{};
    }
    NEFORCE_NODISCARD NEFORCE_CONST_FUNCTION NEFORCE_ALWAYS_INLINE constexpr const_iterator begin() const noexcept {
        return const_iterator{};
    }
    NEFORCE_NODISCARD NEFORCE_CONST_FUNCTION NEFORCE_ALWAYS_INLINE constexpr const_iterator end() const noexcept {
        return const_iterator{};
    }
    NEFORCE_NODISCARD NEFORCE_CONST_FUNCTION NEFORCE_ALWAYS_INLINE constexpr reverse_iterator rbegin() noexcept {
        return reverse_iterator(end());
    }
    NEFORCE_NODISCARD NEFORCE_CONST_FUNCTION NEFORCE_ALWAYS_INLINE constexpr reverse_iterator rend() noexcept {
        return reverse_iterator(begin());
    }
    NEFORCE_NODISCARD NEFORCE_CONST_FUNCTION NEFORCE_ALWAYS_INLINE constexpr const_reverse_iterator
    ...
    NEFORCE_NODISCARD NEFORCE_ALWAYS_INLINE constexpr const_iterator cbegin() const noexcept { return begin(); }
    NEFORCE_NODISCARD NEFORCE_ALWAYS_INLINE constexpr const_iterator cend() const noexcept { return end(); }
    NEFORCE_NODISCARD NEFORCE_ALWAYS_INLINE constexpr const_reverse_iterator crbegin() const noexcept {
        return rbegin();
    }
    NEFORCE_NODISCARD NEFORCE_ALWAYS_INLINE constexpr const_reverse_iterator crend() const noexcept { return rend(); }
    ...
    NEFORCE_NODISCARD NEFORCE_CONST_FUNCTION NEFORCE_ALWAYS_INLINE constexpr T* data() noexcept { return nullptr; }
    NEFORCE_NODISCARD NEFORCE_CONST_FUNCTION NEFORCE_ALWAYS_INLINE constexpr const T* data() const noexcept {
        return nullptr;
    }
