#ifndef NEFORCE_CORE_CONTAINER_MULTIMAP_HPP__
#define NEFORCE_CORE_CONTAINER_MULTIMAP_HPP__
...
private:
    using base_type =
            rb_tree<Key, pair<const Key, T>, select1st<pair<const Key, T>>, Compare, Alloc>; ///< 底层红黑树类型
...
public:
     ...
     */
    void swap(multimap& other) noexcept(is_nothrow_swappable_v<base_type>) { tree_.swap(other.tree_); }
...
multimap(Iterator, Iterator, Compare = Compare(), Alloc = Alloc())
        -> multimap<iter_map_key_t<Iterator>, iter_map_value_t<Iterator>, Compare, Alloc>;

template <typename Key, typename T, typename Compare = less<Key>, typename Alloc = allocator<pair<const Key, T>>>
multimap(std::initializer_list<pair<Key, T>>, Compare = Compare(), Alloc = Alloc()) -> multimap<Key, T, Compare, Alloc>;

template <typename Iterator, typename Alloc>
multimap(Iterator, Iterator, Alloc)
        -> multimap<iter_map_key_t<Iterator>, iter_map_value_t<Iterator>, less<iter_map_key_t<Iterator>>, Alloc>;
