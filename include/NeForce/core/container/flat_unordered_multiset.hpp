#ifndef NEFORCE_CORE_CONTAINER_FLAT_UNORDERED_MULTISET_HPP__
#define NEFORCE_CORE_CONTAINER_FLAT_UNORDERED_MULTISET_HPP__

/**
 * @file flat_unordered_multiset.hpp
 * @brief 平坦无序多重集合容器
 *
 * 此文件提供了平坦无序多重集合容器的实现。
 * flat_unordered_multiset 是一种基于开放寻址哈希表的关联容器，包含键的集合，允许重复键。
 ...
 * @tparam Alloc 分配器类型
 *
 * flat_unordered_multiset 是一种关联容器，存储键值，允许重复键。
 * 元素无序存储，由哈希函数将键映射到槽位。
...
class flat_unordered_multiset : public icollector<flat_unordered_multiset<Value, HashFcn, EqualKey, Alloc>> {
    static_assert(is_hash_v<HashFcn, Value>, "flat_unordered_multiset requires valid hash function.");
    static_assert(is_allocator_v<Alloc>, "Alloc type is not a standard allocator type.");
    static_assert(is_object_v<Value>, "flat_unordered_multiset only contains object types.");

private:
    using base_type = flat_hashtable<Value, Value, HashFcn, identity<Value>, EqualKey,
                                     Alloc>; ///< 底层平坦哈希表类型
...
public:
     ...
     * @brief 默认构造函数
     *
     * 构造一个空 flat_unordered_multiset。
     */
    flat_unordered_multiset() = default;

    /**
     * @brief 构造函数，指定初始容量
     * @param n 初始容量提示
     */
    explicit flat_unordered_multiset(const size_type n) :
    ht_(n) {}
     ...
     * @param hf 哈希函数
     */
    flat_unordered_multiset(const size_type n, const hasher& hf) :
    ht_(n, hf) {}
     ...
     * @param eql 键相等比较函数
     */
    flat_unordered_multiset(const size_type n, const hasher& hf, const key_equal& eql) :
    ht_(n, hf, eql) {}

    /**
     * @brief 拷贝构造函数
     * @param other 源 flat_unordered_multiset
     */
    flat_unordered_multiset(const flat_unordered_multiset& other) :
    ht_(other.ht_) {}
     ...
     * @return 自身引用
     */
    flat_unordered_multiset& operator=(const flat_unordered_multiset& other) {
        ht_ = other.ht_;
        return *this;
    }
     ...
     * @param other 源 flat_unordered_multiset
     */
    flat_unordered_multiset(flat_unordered_multiset&& other) noexcept(is_nothrow_move_constructible_v<base_type>) :
     ...
     * @return 自身引用
     */
    flat_unordered_multiset&
    operator=(flat_unordered_multiset&& other) noexcept(is_nothrow_move_assignable_v<base_type>) {
        ht_ = _NEFORCE move(other.ht_);
        return *this;
    }
    ...
    flat_unordered_multiset(Iterator first, Iterator last) :
    ht_() {
     ...
     */
    template <typename Iterator>
    flat_unordered_multiset(Iterator first, Iterator last, const size_type n) :
    ht_(n) {
        ht_.insert_equal(first, last);
    }
     ...
     */
    template <typename Iterator>
    flat_unordered_multiset(Iterator first, Iterator last, const size_type n, const hasher& hf) :
    ht_(n, hf) {
        ht_.insert_equal(first, last);
    }
     ...
     */
    template <typename Iterator>
    flat_unordered_multiset(Iterator first, Iterator last, const size_type n, const hasher& hf, const key_equal& eql) :
    ht_(n, hf, eql) {
        ht_.insert_equal(first, last);
    }
     ...
     * @param ilist 初始化列表
     */
    flat_unordered_multiset(std::initializer_list<value_type> ilist) :
    flat_unordered_multiset(ilist.begin(), ilist.end()) {}
     ...
     * @param n 初始容量提示
     */
    flat_unordered_multiset(std::initializer_list<value_type> ilist, const size_type n) :
    flat_unordered_multiset(ilist.begin(), ilist.end(), n) {}
     ...
     * @param hf 哈希函数
     */
    flat_unordered_multiset(std::initializer_list<value_type> ilist, const size_type n, const hasher& hf) :
    flat_unordered_multiset(ilist.begin(), ilist.end(), n, hf) {}
    ...
    flat_unordered_multiset(std::initializer_list<value_type> ilist, const size_type n, const hasher& hf,
                            const key_equal& eql) :
    flat_unordered_multiset(ilist.begin(), ilist.end(), n, hf, eql) {}
     ...
     * @param ilist 初始化列表
     */
    void insert(std::initializer_list<value_type> ilist) { ht_.insert_equal(ilist); }
     ...
     * @param other 要交换的容器
     */
    void swap(flat_unordered_multiset& other) noexcept(is_nothrow_swappable_v<base_type>) { ht_.swap(other.ht_); }
     ...
     * @return 是否相等
     */
    NEFORCE_NODISCARD bool equal_to(const flat_unordered_multiset& rhs) const noexcept(noexcept(ht_ == rhs.ht_)) {
        return ht_ == rhs.ht_;
    }
     ...
     * @return 按字典序比较结果
     */
    NEFORCE_NODISCARD bool less_than(const flat_unordered_multiset& rhs) const noexcept(noexcept(ht_ < rhs.ht_)) {
        return ht_ < rhs.ht_;
    }
};
...
flat_unordered_multiset(Iterator, Iterator, HashFcn = HashFcn(), Compare = Compare(), Alloc = Alloc())
        -> flat_unordered_multiset<iter_value_t<Iterator>, HashFcn, Compare, Alloc>;

template <typename Value, typename HashFcn = hash<Value>, typename Compare = equal_to<Value>,
          typename Alloc = allocator<Value>>
flat_unordered_multiset(std::initializer_list<Value>, HashFcn = HashFcn(), Compare = Compare(), Alloc = Alloc())
        -> flat_unordered_multiset<Value, HashFcn, Compare, Alloc>;

template <typename Iterator, typename Alloc>
flat_unordered_multiset(Iterator, Iterator, Alloc)
        -> flat_unordered_multiset<iter_value_t<Iterator>, hash<iter_value_t<Iterator>>,
                                   equal_to<iter_value_t<Iterator>>, Alloc>;

template <typename Iterator, typename HashFcn, typename Alloc>
flat_unordered_multiset(Iterator, Iterator, HashFcn, Alloc)
        -> flat_unordered_multiset<iter_value_t<Iterator>, HashFcn, equal_to<iter_value_t<Iterator>>, Alloc>;

template <typename Value, typename Alloc>
flat_unordered_multiset(std::initializer_list<Value>, Alloc)
        -> flat_unordered_multiset<Value, hash<Value>, equal_to<Value>, Alloc>;

template <typename Value, typename HashFcn, typename Alloc>
flat_unordered_multiset(std::initializer_list<Value>, HashFcn, Alloc)
        -> flat_unordered_multiset<Value, HashFcn, equal_to<Value>, Alloc>;
#endif

/** @} */ // Container

NEFORCE_END_NAMESPACE__
#endif // NEFORCE_CORE_CONTAINER_FLAT_UNORDERED_MULTISET_HPP__
