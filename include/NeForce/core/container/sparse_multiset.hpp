#ifndef NEFORCE_CORE_CONTAINER_SPARSE_MULTISET_HPP__
#define NEFORCE_CORE_CONTAINER_SPARSE_MULTISET_HPP__

/**
 * @file sparse_multiset.hpp
 * @brief 稀疏多重集合容器
 *
 * 此文件提供了稀疏多重集合容器的实现。
 * sparse_multiset是一种有序关联容器，包含键的集合，允许重复键。
...
/**
 * @class sparse_multiset
 * @brief 稀疏多重集合容器
 * @tparam Key 键类型
 * @tparam Compare 键比较函数类型，默认为less<Key>
 * @tparam Alloc 分配器类型
 *
 * sparse_multiset是一种关联容器，存储键值，允许重复键。
...
class sparse_multiset : icollector<sparse_multiset<Key, Compare, Alloc>> {
    static_assert(is_allocator_v<Alloc>, "Alloc type is not a standard allocator type.");
    static_assert(is_same_v<Key, typename Alloc::value_type>, "allocator type mismatch.");
    static_assert(is_object_v<Key>, "sparse_multiset only contains object types.");
...
public:
     ...
     * @brief 默认构造函数
     *
     * 构造一个空sparse_multiset，使用默认的比较函数。
     */
    sparse_multiset() :
    data_(Compare()) {}
     ...
     * @param comp 比较函数对象
     */
    explicit sparse_multiset(const key_compare& comp) :
    data_(comp) {}

    /**
     * @brief 拷贝构造函数
     * @param other 源sparse_multiset
     */
    sparse_multiset(const sparse_multiset& other) :
    data_(other.data_) {}
     ...
     * @return 自身引用
     */
    sparse_multiset& operator=(const sparse_multiset& other) {
        data_ = other.data_;
        return *this;
    }
     ...
     * @param other 源sparse_multiset
     */
    sparse_multiset(sparse_multiset&& other) noexcept(is_nothrow_move_constructible_v<base_type>) :
     ...
     * @return 自身引用
     */
    sparse_multiset& operator=(sparse_multiset&& other) noexcept(is_nothrow_move_assignable_v<base_type>) {
        data_ = _NEFORCE move(other.data_);
        return *this;
    }
    ...
    sparse_multiset(Iterator first, Iterator last) :
    data_(Compare()) {
     ...
     */
    template <typename Iterator>
    sparse_multiset(Iterator first, Iterator last, const key_compare& comp) :
    data_(comp) {
        data_.insert_equal(first, last);
    }
     ...
     * @param ilist 初始化列表
     */
    sparse_multiset(std::initializer_list<value_type> ilist) :
    sparse_multiset(ilist.begin(), ilist.end()) {}
     ...
     * @param comp 比较函数对象
     */
    sparse_multiset(std::initializer_list<value_type> ilist, const key_compare& comp) :
    sparse_multiset(ilist.begin(), ilist.end(), comp) {}
     ...
     * @return 自身引用
     */
    sparse_multiset& operator=(std::initializer_list<value_type> ilist) {
        clear();
     ...
     * @return 指向最大元素的反向迭代器
     */
    NEFORCE_NODISCARD reverse_iterator rbegin() noexcept { return reverse_iterator(data_.end()); }

    /**
     * @brief 获取反向结束迭代器
     * @return 指向最小元素之前位置的反向迭代器
     */
    NEFORCE_NODISCARD reverse_iterator rend() noexcept { return reverse_iterator(data_.begin()); }

    /**
     * @brief 获取常量反向起始迭代器
     * @return 指向最大元素的常量反向迭代器
     */
    NEFORCE_NODISCARD const_reverse_iterator rbegin() const noexcept { return const_reverse_iterator(data_.cend()); }

    /**
     * @brief 获取常量反向结束迭代器
     * @return 指向最小元素之前位置的常量反向迭代器
     */
    NEFORCE_NODISCARD const_reverse_iterator rend() const noexcept { return const_reverse_iterator(data_.cbegin()); }

    /**
     * @brief 获取常量反向起始迭代器
     * @return 指向最大元素的常量反向迭代器
     */
    NEFORCE_NODISCARD const_reverse_iterator crbegin() const noexcept { return const_reverse_iterator(data_.cend()); }

    /**
     * @brief 获取常量反向结束迭代器
     * @return 指向最小元素之前位置的常量反向迭代器
     */
    NEFORCE_NODISCARD const_reverse_iterator crend() const noexcept { return const_reverse_iterator(data_.cbegin()); }

    /**
     * @brief 获取元素数量
     * @return sparse_multiset中的元素数量
     */
    ...
    /**
     * @brief 交换两个sparse_multiset的内容
     * @param other 要交换的另一个sparse_multiset
     */
    void swap(sparse_multiset& other) noexcept(is_nothrow_swappable_v<base_type>) { data_.swap(other.data_); }

    /**
     * @brief 相等比较操作符
     * @param rhs 右侧sparse_multiset
     * @return 如果两个sparse_multiset大小相等且对应元素相等返回true
     */
    NEFORCE_NODISCARD bool equal_to(const sparse_multiset& rhs) const noexcept(noexcept(data_ == rhs.data_)) {
     ...
     * @return 按字典序比较结果
     */
    NEFORCE_NODISCARD bool less_than(const sparse_multiset& rhs) const noexcept(noexcept(data_ < rhs.data_)) {
        return data_ < rhs.data_;
    }
};
...
sparse_multiset(Iterator, Iterator, Compare = Compare(), Alloc = Alloc())
        -> sparse_multiset<iter_value_t<Iterator>, Compare, Alloc>;

template <typename Key, typename Compare = less<Key>, typename Alloc = allocator<Key>>
sparse_multiset(std::initializer_list<Key>, Compare = Compare(), Alloc = Alloc())
        -> sparse_multiset<Key, Compare, Alloc>;

template <typename Iterator, typename Alloc>
sparse_multiset(Iterator, Iterator, Alloc)
        -> sparse_multiset<iter_value_t<Iterator>, less<iter_value_t<Iterator>>, Alloc>;

template <typename Key, typename Alloc>
sparse_multiset(std::initializer_list<Key>, Alloc) -> sparse_multiset<Key, less<Key>, Alloc>;
#endif

/** @} */ // Container

NEFORCE_END_NAMESPACE__
#endif // NEFORCE_CORE_CONTAINER_SPARSE_MULTISET_HPP__
