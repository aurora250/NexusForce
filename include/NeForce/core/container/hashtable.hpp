 ...
 *   §6.4 Hashing（哈希表经典理论分析）
 * - **Thomas H. Cormen, Charles E. Leiserson, Ronald L. Rivest, Clifford Stein**：
 *   Introduction to Algorithms (3rd Edition), Chapter 11 — Hash Tables
 *
 * **哈希函数设计参考：**
 * - **Maurice V. Wilkes (1965)**：Slave Memories and Dynamic Storage Allocation
 ...
 *
 * **负载因子与性能分析：**
 * - **Gaston H. Gonnet, Per-Åke Larson (1988)**：External hashing with limited internal storage
 *   Journal of the ACM, 35(1): 161-184
 *
 * @section hash_table_principles 哈希表原理
 ...
 * - 查找时需要遍历链表
 *
 * @section bucket_size_strategy 桶大小策略
 ...
 * - 改善分布均匀性（参考 Knuth 分析）
 *
 * @section load_factor_management 负载因子管理
 ...
 * | 节点分配          | 使用 Alloc 分配器                           |
 *
 * @section iterator_invalidation 迭代器失效规则
 * | 操作           | 失效范围                                    |
 * |----------------|--------------------------------------------|
 * | insert         | 不失效任何迭代器                             |
 ...
 * | swap           | 迭代器交换，指向元素保持有效                   |
 *
 * @section unique_vs_equal 唯一键与允许重复键
 * 本实现提供两种插入模式：
 *
 * | 模式          | 方法后缀      | 行为                                   |
 * |---------------|---------------|----------------------------------------|
 ...
 *
 * @note 本实现采用链地址法处理冲突，在负载因子适当时性能最优。
 *       对于需要稳定迭代器的场景，哈希表是理想选择。
 *
 * @see https://en.cppreference.com/w/cpp/container/unordered_map
 * @see https://en.wikipedia.org/wiki/Hash_table
 * @see https://dl.acm.org/doi/10.1145/356643.356645
 * @{
 */
    ...
    pair<iterator, bool> insert_unique_noresize(link_type ptr) {
        ...
        while (*buckets_ptr != nullptr) {
            if (equals_(extracter_((*buckets_ptr)->data), extracter_(ptr->data))) {
                link_type existing = *buckets_ptr;
                hashtable::delete_node(ptr);
                return {iterator(existing, n, this), false};
            }
...
public:
    ...
    NEFORCE_NODISCARD pair<iterator, iterator> equal_range(const key_type& key) {
        ...
        link_type first = buckets_[n];
        while (first && !equals_(extracter_(first->data), key)) {
            first = first->next;
        }
        ...
        while (last && equals_(extracter_(last->data), key)) {
            last = last->next;
        }

        iterator second_it;
        if (last != nullptr) {
            second_it = iterator(last, n, this);
        } else {
            size_type next_bucket = n + 1;
            while (next_bucket < buckets_.size() && buckets_[next_bucket] == nullptr) {
                ++next_bucket;
            }
            if (next_bucket < buckets_.size()) {
                second_it = iterator(buckets_[next_bucket], next_bucket, this);
            } else {
                second_it = end();
            }
        }

        return {iterator(first, n, this), second_it};
    }
    ...
    NEFORCE_NODISCARD pair<const_iterator, const_iterator> equal_range(const key_type& key) const {
        ...
        if (last != nullptr) {
            second_it = const_iterator(last, n, this);
        } else {
            size_type next_bucket = n + 1;
            while (next_bucket < buckets_.size() && buckets_[next_bucket] == nullptr) {
                ++next_bucket;
            ...
            if (next_bucket < buckets_.size()) {
                second_it = const_iterator(buckets_[next_bucket], next_bucket, this);
            } else {
                second_it = cend();
            }
        }

        return {const_iterator(first, n, this), second_it};
    }
