...
public:
     ...
     * @return 自身引用
     */
    unordered_multimap& operator=(unordered_multimap&& other) noexcept(is_nothrow_move_assignable_v<base_type>) {
        ht_ = _NEFORCE move(other.ht_);
        return *this;
    }
     ...
     */
    void swap(unordered_multimap& other) noexcept(is_nothrow_swappable_v<base_type>) { ht_.swap(other.ht_); }
     ...
     */
    NEFORCE_NODISCARD bool equal_to(const unordered_multimap& rhs) const noexcept(noexcept(ht_ == rhs.ht_)) {
        return ht_ == rhs.ht_;
    }
     ...
     * @return 按字典序比较结果
     */
    NEFORCE_NODISCARD bool less_than(const unordered_multimap& rhs) const noexcept(noexcept(ht_ < rhs.ht_)) {
        return ht_ < rhs.ht_;
    }
};
...
unordered_multimap(Iterator, Iterator, Alloc)
        -> unordered_multimap<iter_map_key_t<Iterator>, iter_map_value_t<Iterator>, hash<iter_map_key_t<Iterator>>,
                              equal_to<iter_map_key_t<Iterator>>, Alloc>;

template <typename Iterator, typename HashFcn, typename Alloc>
unordered_multimap(Iterator, Iterator, HashFcn, Alloc)
        -> unordered_multimap<iter_map_key_t<Iterator>, iter_map_value_t<Iterator>, HashFcn,
                              equal_to<iter_map_key_t<Iterator>>, Alloc>;
