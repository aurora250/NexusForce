#ifndef NEFORCE_CORE_CONTAINER_BITMAP_HPP__
#define NEFORCE_CORE_CONTAINER_BITMAP_HPP__
...
public:
    NEFORCE_CONSTEXPR20 bitmap_iterator() noexcept = default;
    ...
    NEFORCE_NODISCARD NEFORCE_CONSTEXPR20 reference dereference() const noexcept {
        NEFORCE_DEBUG_VERIFY(ptr_ && container_, "Attempting to dereference on a null pointer");
    ...
    NEFORCE_CONSTEXPR20 void increment() noexcept {
        NEFORCE_DEBUG_VERIFY(ptr_ && container_, "Attempting to increment a null pointer");
...
private:
    ...
    struct bit_storage {
         ...
         * @brief 默认构造函数
         */
        NEFORCE_CONSTEXPR20 bit_storage() noexcept = default;
...
private:
     ...
     * @param word 位数
     */
    NEFORCE_CONSTEXPR20 void set_iterators(const size_type word) noexcept {
    ...
    NEFORCE_CONSTEXPR20 Iterator2 bit_copy(Iterator1 first, Iterator1 last, Iterator2 result) {
     ...
     */
    template <typename Iterator1, typename Iterator2>
    NEFORCE_CONSTEXPR20 Iterator2 bit_copy_backward(Iterator1 first, Iterator1 last, Iterator2 result) {
        iter_difference_t<Iterator1> n = _NEFORCE distance(first, last);
        for (; n > 0; --n) {
            *--result = *--last;
        }
    ...
    enable_if_t<is_ranges_fwd_iter_v<Iterator>> range_init(Iterator first, Iterator last) {
        ...
        storage_ = _NEFORCE move(tmp_storage);
        start_ = tmp_start;
        finish_ = tmp_finish;
        return;
    }
