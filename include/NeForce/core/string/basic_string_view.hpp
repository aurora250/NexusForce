#ifndef NEFORCE_CORE_STRING_BASIC_STRING_VIEW_HPP__
#define NEFORCE_CORE_STRING_BASIC_STRING_VIEW_HPP__
...
public:
    ...
    NEFORCE_NODISCARD constexpr reference dereference() const noexcept {
        NEFORCE_DEBUG_VERIFY(data_, "Attempting to dereference on a null pointer");
        NEFORCE_DEBUG_VERIFY(idx_ < size_, "Attempting to dereference out of boundary");
        return data_[idx_];
    }
    ...
    constexpr void increment() noexcept {
        NEFORCE_DEBUG_VERIFY(data_, "Attempting to increment a null pointer");
        NEFORCE_DEBUG_VERIFY(idx_ < size_, "Attempting to increment out of boundary");
        ++idx_;
    }
    ...
    constexpr void decrement() noexcept {
        NEFORCE_DEBUG_VERIFY(data_, "Attempting to decrement a null pointer");
        NEFORCE_DEBUG_VERIFY(idx_ != 0, "Attempting to decrement out of boundary");
        --idx_;
    }
     ...
     * @return 是否相等
     */
    NEFORCE_NODISCARD constexpr bool equal_to(const basic_string_view_iterator& rhs) const noexcept {
        NEFORCE_DEBUG_VERIFY(data_ == rhs.data_ && size_ == rhs.size_, "Attempting to equal to a different container");
...
public:
    ...
    NEFORCE_NODISCARD constexpr const_reference front() const noexcept {
        NEFORCE_DEBUG_VERIFY(!empty(), "cannot call front on empty string_view");
        return data_[0];
    }
    ...
    NEFORCE_NODISCARD constexpr const_reference back() const noexcept {
        NEFORCE_DEBUG_VERIFY(!empty(), "cannot call back on empty string_view");
    ...
    constexpr void remove_prefix(const size_type n) noexcept {
        NEFORCE_DEBUG_VERIFY(size_ >= n, "cannot remove prefix longer than total size");
        data_ += n;
        size_ -= n;
    }
    ...
    constexpr void remove_suffix(const size_type n) noexcept {
        NEFORCE_DEBUG_VERIFY(size_ >= n, "cannot remove suffix longer than total size");
        size_ -= n;
    }
     ...
     * @return 子串
     */
    NEFORCE_NODISCARD constexpr basic_string_view head(const size_type count = npos) const { return substr(0, count); }
     ...
     * @return 子串
     */
    NEFORCE_NODISCARD constexpr basic_string_view tail(const size_type off = 0) const { return substr(off); }
...
#endif // NEFORCE_CORE_STRING_BASIC_STRING_VIEW_HPP__
