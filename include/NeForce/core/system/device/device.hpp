#ifndef NEFORCE_CORE_SYSTEM_DEVICE_DEVICE_HPP__
#define NEFORCE_CORE_SYSTEM_DEVICE_DEVICE_HPP__
...
class NEFORCE_API device {
public:
    using async_result = file::async_result;
    using device_handle = file::native_handle_type;
...
public:
    ...
    _NEFORCE device_info device_info() const;
    ...
    static vector<_NEFORCE device_info> enumerate(const string& filter = "");

    static vector<_NEFORCE device_info> find_by_vid_pid(uint16_t vid, uint16_t pid);
    static optional<_NEFORCE device_info> find_by_path(path pth);
...
#endif // NEFORCE_CORE_SYSTEM_DEVICE_DEVICE_HPP__
