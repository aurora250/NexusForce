#ifndef NEFORCE_CORE_FUNCTIONAL_FUNCTION_HPP__
...
union storage_data {
	...
	template <typename T>
	NEFORCE_NODISCARD T& access() noexcept { return *static_cast<T*>(access()); }

	template <typename T>
	NEFORCE_NODISCARD const T& access() const noexcept { return *static_cast<const T*>(access()); }
