#ifndef NEFORCE_CORE_ALGORITHM_EXT_SORT_HPP__
#define NEFORCE_CORE_ALGORITHM_EXT_SORT_HPP__
...
NEFORCE_CONSTEXPR20 void counting_sort(Iterator first, Iterator last, Compare comp, IndexMapper mapper) {
    ...
    for (auto rit = _NEFORCE make_reverse_iterator(last); rit != bound; ++rit) {
        auto value = mapper(*rit);
        const auto index = static_cast<size_t>(value - min_val);
        size_t position = --count[index];
        sorted[position] = *rit;
    }
    _NEFORCE copy(sorted.begin(), sorted.end(), first);
}
...
int __max_bit_aux(Iterator first, Iterator last) {
    auto max_num = *_NEFORCE max_element(first, last,
...
NEFORCE_CONSTEXPR20 void radix_sort(Iterator first, Iterator last, Mapper mapper = Mapper()) {
    _NEFORCE radix_sort_less(first, last, mapper);
}
