...
public:
	...
	constexpr tuple(Tag, U1&& this_arg, U2&&... rest_arg)
		: base_type(exact_arg_construct_tag{}, _NEFORCE forward<U2>(rest_arg)...), data_(_NEFORCE forward<U1>(this_arg)) {}
	...
	NEFORCE_CONSTEXPR14 tuple& operator =(type_identity_t<const tuple&> tup) noexcept(conjunction<
	...
	NEFORCE_CONSTEXPR14 tuple& operator =(type_identity_t<tuple&&> tup) noexcept(conjunction<
	 ...
	 * @param t 要交换的元组
	 */
	NEFORCE_CONSTEXPR14 void swap(tuple& t)
...
struct constructible_from_tuple<T, Tuple, index_sequence<Index...>> : bool_constant<
	is_constructible<T, decltype(_NEFORCE get<Index>(_NEFORCE declval<Tuple>()))...>::value> {};
 ...
 */
template <typename T, typename Tuple, size_t... Index>
NEFORCE_NODISCARD constexpr T __broaden_make_from_tuple(Tuple&& tup, index_sequence<Index...> idx)
noexcept(is_nothrow_constructible<T, decltype(_NEFORCE get<Index>(_NEFORCE forward<Tuple>(tup)))...>::value) {
