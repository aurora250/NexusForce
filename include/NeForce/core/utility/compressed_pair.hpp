...
struct compressed_pair final : IfEmpty, icommon<compressed_pair<IfEmpty, T, Compressed>> {
	 ...
	 * @return 当前压缩对的引用
	 */
    constexpr compressed_pair& operator =(const compressed_pair& pir)
	...
	: IfEmpty(), value(_NEFORCE forward<Args>(args)...) {}
	 ...
	 * 使用给定的参数构造基类和值。
	 */
    template <typename ToEmpty, typename... Args, enable_if_t<conjunction_v<
    	is_constructible<IfEmpty, ToEmpty>, is_constructible<T, Args...>>, int> = 0>
	...
	: IfEmpty(_NEFORCE forward<ToEmpty>(first)), value(_NEFORCE forward<Args>(args)...) {}
...
struct compressed_pair<IfEmpty, T, false> final : icommon<compressed_pair<IfEmpty, T, false>> {
	...
	noexcept(conjunction_v<
		is_nothrow_copy_constructible<IfEmpty>,
	...
	constexpr compressed_pair& operator =(const compressed_pair& pir)
	noexcept(conjunction_v<
        is_nothrow_copy_assignable<IfEmpty>,
	...
	noexcept(conjunction_v<
		is_nothrow_move_constructible<IfEmpty>,
		is_nothrow_move_constructible<T>>)
	: no_compressed(_NEFORCE move(pir.no_compressed)), value(_NEFORCE move(pir.value)) {}
    ...
    constexpr compressed_pair& operator =(compressed_pair&& pir)
	noexcept(conjunction_v<
        is_nothrow_move_assignable<IfEmpty>,
	...
	: no_compressed(), value(_NEFORCE forward<Args>(args)...) {}
	...
	noexcept(conjunction_v<
		...
		is_nothrow_constructible<T, Args...>>)
	: no_compressed(_NEFORCE forward<ToEmpty>(first)), value(_NEFORCE forward<Args>(args)...) {}
