...
struct missing_required_exception final : deserialize_exception {
    explicit missing_required_exception(const char* property_name, const int code = 0) noexcept :
...
struct type_mismatch_exception final : deserialize_exception {
    explicit type_mismatch_exception(const char* expected_type, const char* actual_type, const int code = 0) noexcept :
