 ...
 * @brief 期望值操作异常
 */
NEFORCE_ERROR_BUILD_FINAL_CLASS(future_exception, exception, "Future Operation Failed.")
...
struct __future_base {
    ...
    public:
         ...
         */
        struct Deleter {
            void operator ()(result_base* result) const {
                result->destroy();
            }
        };
    };
    ...
    allocate_result(const _NEFORCE allocator<T>&) {
    ...
    public:
         ...
         */
        state_base() noexcept
        : status(status::not_ready), retrieved(false) {
            retrieved.clear();
        }
        ...
        void break_promise(PtrType result) {
            if (static_cast<bool>(result)) {
                result->error_ptr = make_exception_ptr(
                    future_exception(future_errc_cstr(future_errc::broken_promise)));
         ...
         */
        template <typename Res, typename Arg>
        NEFORCE_ALWAYS_INLINE static setter<Res, Arg&&>
         ...
         */
        template <typename Res>
        NEFORCE_ALWAYS_INLINE static setter<Res, exception_ptr_tag>
         ...
         */
        template <typename Res>
        NEFORCE_ALWAYS_INLINE static setter<Res, void>
    ...
    private:
         ...
         * @return 是否是延迟future
         */
        virtual bool is_deferred_future() const {
            return false;
        }
...
public:
    ...
    void set(Res& result) noexcept {
        value_ptr = _NEFORCE addressof(result);
    }
     ...
     * @return 引用结果
     */
    NEFORCE_NODISCARD Res& get() noexcept {
        return *value_ptr;
    }
};
...
public:
    ...
    future(future&& other) noexcept : base_type(_NEFORCE move(other)) {}
...
public:
    ...
    shared_future(future<Res>&& other) noexcept : base_type(_NEFORCE move(other)) {}
    shared_future(shared_future&& other) noexcept : base_type(_NEFORCE move(other)) {}
...
public:
    ...
    shared_future(const shared_future& other) : base_type(other) {}
    shared_future(future<void>&& other) noexcept : base_type(_NEFORCE move(other)) {}
    shared_future(shared_future&& other) noexcept : base_type(_NEFORCE move(other)) {}
...
inline shared_future<void> future<void>::share() noexcept {
    return shared_future<void>(_NEFORCE move(*this));
}
