#ifndef NEFORCE_CORE_TYPEINFO_TYPE_TRAITS_HPP__
#define NEFORCE_CORE_TYPEINFO_TYPE_TRAITS_HPP__
...
struct integral_constant {
     ...
     * @return 存储的整数值
     */
    NEFORCE_NODISCARD constexpr value_type operator()() const noexcept { return value; }
};
...
struct disjunction<First, Rest...> : inner::__disjunction_aux<static_cast<bool>(First::value), First, Rest...>::type {};
/// @endcond
...
/// @cond
template <typename First, typename... Rest>
struct conjunction<First, Rest...> : inner::__conjunction_aux<static_cast<bool>(First::value), First, Rest...>::type {};
/// @endcond
...
NEFORCE_MACRO_RANGES_CV_REF(__NEFORCE_EXPAND_REM_FUNC_QULF)
#ifdef NEFORCE_STANDARD_17
NEFORCE_MACRO_RANGES_CV_REF_NOEXCEPT(__NEFORCE_EXPAND_REM_FUNC_QULF)
#endif
#undef __NEFORCE_EXPAND_REM_FUNC_QULF
 ...
 */
template <typename T>
struct is_character : bool_constant<is_any_of<remove_cvref_t<T>, char, wchar_t,
#ifdef NEFORCE_STANDARD_20
                                              char8_t,
#endif
                                              char16_t, char32_t>::value> {
};
...
/// @cond
template <typename T>
struct get_ptr_difference<T, enable_if_t<is_same<typename T::difference_type, typename T::difference_type>::value>> {
...
/// @cond
template <typename T, typename U>
struct get_rebind_type<T, U,
                       enable_if_t<is_same<typename T::template rebind<U>, typename T::template rebind<U>>::value>> {
 ...
 */
template <typename T>
struct is_function : bool_constant<!is_const<const remove_function_qualifiers_t<T>>::value &&
...
struct is_scalar
: bool_constant<
          disjunction<is_arithmetic<T>, is_enum<T>, is_pointer<T>, is_member_pointer<T>, is_null_pointer<T>>::value> {};
...
#    ifdef NEFORCE_STANDARD_14
template <typename T1, typename T2>
NEFORCE_INLINE17 constexpr bool is_layout_compatible_v = is_layout_compatible<T1, T2>::value;
#    endif
 ...
 */
template <typename Base, typename Derived>
NEFORCE_INLINE17 constexpr bool is_pointer_interconvertible_base_of_v =
 ...
 */
template <typename T>
struct is_copy_constructible : bool_constant<is_constructible<T, add_lvalue_reference_t<const T>>::value> {};
...
/// @cond
template <typename T>
struct is_implicitly_default_constructible<T, void_t<decltype(inner::__implicitly_default_construct_aux<T>({}))>>
: true_type {};
/// @endcond
...
template <typename T, bool = disjunction<is_void<T>, is_unbounded_array<T>, is_function<T>>::value,
 ...
 */
template <typename T>
struct is_trivially_move_assignable : bool_constant<is_trivially_assignable<add_lvalue_reference_t<T>, T>::value> {};
 ...
 */
template <typename T>
struct is_nothrow_move_assignable : bool_constant<is_nothrow_assignable<add_lvalue_reference_t<T>, T>::value> {};
 ...
 */
template <typename T>
NEFORCE_NODISCARD constexpr conditional_t<!is_nothrow_move_constructible<T>::value && is_copy_constructible<T>::value,
                                          const T&, T&&>
 ...
 */
template <typename From, typename To>
concept convertible_to = is_convertible_v<From, To> && requires { static_cast<To>(_NEFORCE declval<From>()); };
 ...
 */
template <typename From, typename To, bool IsConvertible = is_convertible<From, To>::value,
          bool IsVoid = is_void<To>::value>
struct is_nothrow_convertible : bool_constant<noexcept(_NEFORCE declcopy<To>(_NEFORCE declval<From>()))> {};
...
struct is_nothrow_arrow<Iterator, Ptr, false>
...
private:
    ...
    using check_func_t =
            conditional_t<is_function<remove_ref_t>::value, add_pointer_t<remove_ref_t>, remove_cv_t<remove_ref_t>>;

public:
    using type =
            conditional_t<is_array<remove_ref_t>::value, add_pointer_t<remove_extent_t<remove_ref_t>>, check_func_t>;
};
...
struct __common_reference_base_aux<T1, T2> {
    using type = _NEFORCE common_ternary_operator_t<T1, T2>;
};
...
NEFORCE_CONSTEXPR14 enable_if_t<is_swappable<T>::value> swap(T (&lhs)[Size],
                                                             T (&rhs)[Size]) noexcept(is_nothrow_swappable<T>::value);
...
/// @cond
template <typename T1, typename T2>
struct is_swappable_from<T1, T2, void_t<decltype(_NEFORCE swap(_NEFORCE declval<T1>(), _NEFORCE declval<T2>()))>>
: true_type {};
/// @endcond
 ...
 */
template <typename T1, typename T2>
struct is_swappable_with : bool_constant<conjunction<is_swappable_from<T1, T2>, is_swappable_from<T2, T1>>::value> {};
 ...
 */
template <typename T>
struct is_swappable : bool_constant<is_swappable_with<add_lvalue_reference_t<T>, add_lvalue_reference_t<T>>::value> {};
...
: bool_constant<noexcept(_NEFORCE swap(_NEFORCE declval<T1>(), _NEFORCE declval<T2>())) &&
                noexcept(_NEFORCE swap(_NEFORCE declval<T2>(), _NEFORCE declval<T1>()))> {};
...
/// @cond
template <typename T>
struct is_ADL_swappable<T, void_t<decltype(swap(_NEFORCE declval<T&>(), _NEFORCE declval<T&>()))>> : true_type {};
/// @endcond
...
struct is_iterable : bool_constant<inner::__has_valid_begin_end<Container>::value &&
...
struct is_maplike
: bool_constant<is_iterable<Map>::value &&
