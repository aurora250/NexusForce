 ...
 */
template <typename Base, typename Derived>
NEFORCE_INLINE17 constexpr bool is_pointer_interconvertible_base_of_v = is_pointer_interconvertible_base_of<Base, Derived>::value;
#endif
#endif
...
struct is_nothrow_arrow<Iterator, Ptr, false> : bool_constant<
    noexcept(_NEFORCE declcopy<Ptr>(_NEFORCE declval<Iterator>().operator->()))> {};
/// @endcond
...
template <typename T1, typename T2> requires requires {
    typename _NEFORCE common_ternary_operator_t<T1, T2>;
}
...
NEFORCE_CONSTEXPR14 enable_if_t<is_swappable<T>::value>
...
NEFORCE_CONSTEXPR14 T exchange(T& val, U&& new_val)
...
struct is_swappable_from<T1, T2, void_t<decltype(
    _NEFORCE swap(_NEFORCE declval<T1>(), _NEFORCE declval<T2>()))>> : true_type {};
/// @endcond
...
struct is_nothrow_swappable_from : bool_constant<
    noexcept(_NEFORCE swap(_NEFORCE declval<T1>(), _NEFORCE declval<T2>())) &&
...
struct is_ADL_swappable<T, void_t<
    decltype(swap(_NEFORCE declval<T&>(), _NEFORCE declval<T&>()))>> : true_type {};
/// @endcond
...
private:
    ...
    typename = decltype(_NEFORCE declval<Alloc1*>()->construct(_NEFORCE declval<T*>(), _NEFORCE declval<Args>()...))>
    static true_type __test(int);
 ...
 */
template <typename T>
NEFORCE_ALWAYS_INLINE constexpr T initialize()
