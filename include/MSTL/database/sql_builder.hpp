...
struct MSTL_API insert_data {
    vector<string> fields;
    vector<string> placeholders;
};

struct MSTL_API update_data {
    vector<string> assignments;
};


class MSTL_API sql_builder {
private:
...
public:
    ...
    MSTL_NODISCARD SQL_OPERATE_TYPE get_type() const noexcept { return sql_type_; }
    MSTL_NODISCARD string_view get_table() const noexcept { return table_.view(); }
    MSTL_NODISCARD bool empty() const noexcept { return table_.empty(); }

    MSTL_NODISCARD string build() const;
};
