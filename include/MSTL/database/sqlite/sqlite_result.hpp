...
struct MSTL_API sqlite_result final : idb_tb_result {
private:
...
public:
    sqlite_result() noexcept = default;
    explicit sqlite_result(_MSTL_SQLITE sqlite3_stmt* statement) noexcept;

    ~sqlite_result() override { if (stmt_) _MSTL_SQLITE sqlite3_finalize(stmt_); }
