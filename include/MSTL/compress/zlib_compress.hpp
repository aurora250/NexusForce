...
private:
    ...
    MSTL_NODISCARD static byte_vector compress_data(
        const byte_t* data, size_t size, compress_level level, compress_strategy strategy);
...
public:
     ...
     */
    MSTL_NODISCARD static byte_vector compress(
        const string_view data, const compress_level level = compress_level::default_level,
        const compress_strategy strategy = compress_strategy::default_strategy) {
    ...
    template <typename T>
    MSTL_NODISCARD static byte_vector compress(
        const vector<T>& data, const compress_level level = compress_level::default_level,
        const compress_strategy strategy = compress_strategy::default_strategy) {
    ...
    MSTL_NODISCARD static byte_vector decompress(
        Iterator begin, Iterator end, const size_t estimated_original_size = 0) {
        static_assert(sizeof(*begin) == 1, "Iterator must point to byte-sized elements");
     ...
     * @throws zlib_exception 当解压缩失败或超出最大缓冲区限制时抛出
     */
    MSTL_NODISCARD static byte_vector decompress(const cbyte_view& data, const size_t estimated_original_size = 0) {
    ...
    public:
         ...
         * @return 已处理的输入字节数
         */
        MSTL_NODISCARD size_t bytes_input() const noexcept {
            return bytes_input_;
        }
         ...
         * @return 已产生的输出字节数
         */
        MSTL_NODISCARD size_t bytes_output() const noexcept {
