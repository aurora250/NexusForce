...
public:
    ...
    constexpr array(std::initializer_list<T> ilist) noexcept {
        ...
        _MSTL copy(ilist.begin(), ilist.begin() + size, array_);
    }
     ...
     * @return 索引位置元素的引用
     */
    MSTL_NODISCARD constexpr reference at(size_type position) noexcept {
        MSTL_DEBUG_VERIFY(position < Size, "array subscript out of range");
     ...
     * @return 索引位置元素的常量引用
     */
    MSTL_NODISCARD constexpr const_reference at(size_type position) const noexcept {
        MSTL_DEBUG_VERIFY(position < Size, "array subscript out of range");
        return array_[position];
    }
    /**
     ...
     * @return 索引位置元素的引用
     */
    MSTL_NODISCARD constexpr reference operator[](size_type position) noexcept {
        MSTL_DEBUG_VERIFY(position < Size, "array subscript out of range");
        return array_[position];
    }
    /**
     ...
     * @return 索引位置元素的常量引用
     */
    MSTL_NODISCARD constexpr const_reference operator[](size_type position) const noexcept {
        MSTL_DEBUG_VERIFY(position < Size, "array subscript out of range");
        return array_[position];
    }
    ...
    noexcept(is_nothrow_swappable_v<T>) {
        _MSTL swap(array_, other.array_);
    }
...
template <typename T, size_t Size>
MSTL_INLINE17 constexpr size_t tuple_size_v<array<T, Size>> = Size;  ///< 元组大小值

template <typename T, size_t Size>
MSTL_INLINE17 constexpr size_t tuple_size_v<const array<T, Size>> = Size;  ///< 常量数组的元组大小值
