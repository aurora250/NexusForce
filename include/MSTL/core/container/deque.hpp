...
constexpr size_t deque_buf_size(const size_t n, const size_t sz) noexcept {
    ...
    constexpr size_t buffer_max_size = 4096;
...
public:
    ...
    /// 缓冲区大小
    static constexpr difference_type buffer_size = _MSTL deque_buf_size(BufSize, sizeof(value_type));
...
public:
    ...
    MSTL_NODISCARD reference dereference() const noexcept {
        ...
        MSTL_DEBUG_VERIFY(first_ <= current_ && current_ < last_, "Attempting to dereference out of boundary");
        return *current_;
    }
     ...
     * 考虑缓冲区间隔计算准确距离。
     */
    MSTL_NODISCARD MSTL_CONSTEXPR20 difference_type distance_to(const deque_iterator& other) const noexcept {
     ...
     * @return 偏移位置元素的引用
     */
    MSTL_NODISCARD reference operator [](const difference_type n) noexcept {
        return *(*this + n);
    }
     ...
     * @return 是否相等
     */
    MSTL_NODISCARD bool equal(const deque_iterator& rhs) const noexcept {
     ...
     */
    MSTL_NODISCARD bool less_than(const deque_iterator& rhs) const noexcept {
...
private:
    ...
    void create_map_and_nodes(const size_type n) {
        ...
        map_size_pair_.value = _MSTL max(init_map_size, node_nums + 2);
    ...
    void fill_initialize(const size_type n, const T& value) {
        ...
        for (map_pointer cur = start_.node_; cur < finish_.node_; ++cur) {
            _MSTL uninitialized_fill(*cur, *cur + buffer_size, value);
        }
        _MSTL uninitialized_fill(finish_.first_, finish_.current_, value);
    }
    ...
    enable_if_t<is_ranges_fwd_iter_v<Iterator>>
    copy_initialize(Iterator first, Iterator last) {
        deque::create_map_and_nodes(_MSTL distance(first, last));

        for (map_pointer cur = start_.node_; cur < finish_.node_; ++cur) {
            Iterator next = _MSTL next(first, buffer_size);
        ...
        if (dist_before < static_cast<difference_type>(size() / 2)) {
            ...
            try {
                if (dist_before >= n) {
                    ...
                    start_ = new_start;
                    _MSTL copy(start_n, position, old_start);
                    _MSTL fill(position - n, position, value);
                } else {
                    _MSTL uninitialized_fill(
                        _MSTL uninitialized_copy(start_, position, new_start), start_, value);
                    start_ = new_start;
                    _MSTL fill(old_start, position, value);
                }
            } catch (...) {
                if (new_start.node_ != start_.node_) {
                    deque::destroy_nodes(new_start.node_, start_.node_ - 1);
            ...
            try {
                if (dist_after > n) {
                    ...
                    finish_ = new_finish;
                    _MSTL copy_backward(position, finish_n, old_finish);
                    _MSTL fill(position, position + n, value);
                } else {
                    _MSTL uninitialized_fill(finish_, position + n, value);
                    _MSTL uninitialized_copy(position, finish_, position + n);
                    finish_ = new_finish;
                    _MSTL fill(position, old_finish, value);
                }
            } catch (...) {
                if (new_finish.node_ != finish_.node_) {
                    deque::destroy_nodes(finish_.node_ + 1, new_finish.node_);
    ...
    noexcept(is_nothrow_move_assignable_v<value_type>) {
        ...
        if (index < size() / 2) {
            ...
            if (new_pos != finish_ - 1) {
                _MSTL copy_backward(new_pos, finish_ - 1, finish_);
            }

            _MSTL destroy(new_pos.current_);
            _MSTL construct(new_pos.current_, _MSTL forward<Args>(args)...);
            return new_pos;
        } else {
            ...
            iterator new_pos = start_ + index;
            if (new_pos != start_) {
                _MSTL copy_backward(start_, new_pos, new_pos + 1);
            }
...
public:
     ...
     */
    deque() {
        deque::fill_initialize(0, _MSTL initialize<T>());
    }
    ...
    explicit deque(const size_type n) {
        deque::fill_initialize(n, _MSTL initialize<T>());
    }
    ...
    deque& operator =(const deque& other) {
        ...
        if (len >= other.size()) {
            deque::erase(_MSTL copy(other.start_, other.finish_, start_), finish_);
        } else {
            auto mid = other.begin() + len;
            _MSTL copy(other.begin(), mid, start_);
    ...
    void resize(const size_type n) {
        deque::resize(n, _MSTL initialize<T>());
    }
    ...
    iterator emplace(iterator position, Args&&... args) {
        if (position.current_ == start_.current_) {
            deque::emplace_front(_MSTL forward<Args>(args)...);
            return start_;
        }
        if (position.current_ == finish_.current_) {
            deque::emplace_back(_MSTL forward<Args>(args)...);
            return finish_ - 1;
        }
        return deque::insert_aux(position, _MSTL forward<Args>(args)...);
    }
    ...
    void emplace_back(Args&&... args) {
        if (finish_.current_ != finish_.last_ - 1) {
            _MSTL construct(finish_.current_, _MSTL forward<Args>(args)...);
            ++finish_.current_;
        } else {
    ...
    void emplace_front(Args&&... args) {
        if (start_.current_ != start_.first_) {
            _MSTL construct(start_.current_ - 1, _MSTL forward<Args>(args)...);
            --start_.current_;
        } else {
            deque::reallocate_map(1, true);
            --start_;
            _MSTL construct(start_.current_, _MSTL forward<Args>(args)...);
        }
    }
    ...
    void push_back(T&& value) {
        deque::emplace_back(_MSTL move(value));
    }
    ...
    void push_front(T&& value) {
        deque::emplace_front(_MSTL move(value));
    }
    ...
    noexcept(is_nothrow_destructible_v<value_type>) {
        ...
        if (finish_.current_ != finish_.first_) {
            --finish_.current_;
            _MSTL destroy(finish_.current_);
        } else {
            _MSTL destroy(finish_.current_);
    ...
    iterator insert(iterator position, T&& value) {
        ...
        return deque::insert_aux(position, _MSTL move(value));
    }
    ...
    void insert(iterator position, const size_t n, const T& value) {
        if (position.current_ == start_.current_) {
            deque::reallocate_map(n, true);
            auto new_start = start_ - n;
            _MSTL uninitialized_fill_n(new_start, n, value);
            start_ = new_start;
        } else if (position.current_ == finish_.current_) {
            deque::reallocate_map(n, false);
            auto new_finish = finish_ + n;
            _MSTL uninitialized_fill_n(finish_, n, value);
            finish_ = new_finish;
        } else {
    ...
    iterator erase(iterator first, iterator last) {
        ...
        if (dist_before < (size() - len) / 2) {
            ...
            _MSTL destroy(start_.current_, new_start.current_);
            start_ = new_start;
        } else {
            _MSTL copy(last, finish_, first);
            iterator new_finish = finish_ - len;
            _MSTL destroy(new_finish.current_, finish_.current_);
            finish_ = new_finish;
        }
    ...
    noexcept(is_nothrow_destructible_v<value_type>) {
        for (map_pointer cur = start_.node_ + 1; cur < finish_.node_; ++cur) {
            _MSTL destroy(*cur, *cur + buffer_size);
        }

        if (start_.node_ == finish_.node_) {
            _MSTL destroy(start_.current_, finish_.current_);
        } else {
            _MSTL destroy(start_.current_, start_.last_);
            _MSTL destroy(finish_.first_, finish_.current_);
        }
     ...
     * @return 指定位置元素的常量引用
     */
    MSTL_NODISCARD const_reference at(size_type position) const noexcept {
     ...
     * @return 指定位置元素的引用
     */
    MSTL_NODISCARD reference at(const size_type position) noexcept {
        return start_[position];
    }
     ...
     * @return 指定位置元素的常量引用
     */
    MSTL_NODISCARD const_reference operator [](const size_type position) const noexcept {
        return at(position);
    }
     ...
     * @return 指定位置元素的引用
     */
    MSTL_NODISCARD reference operator [](const size_type position) noexcept {
        return at(position);
    }
    ...
    noexcept(is_nothrow_swappable_v<map_allocator> && is_nothrow_swappable_v<allocator_type>) {
        ...
        _MSTL swap(map_pair_, other.map_pair_);
        _MSTL swap(map_size_pair_, other.map_size_pair_);
    }
