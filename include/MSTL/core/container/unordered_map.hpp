#ifndef MSTL_CORE_CONTAINER_UNORDERED_MAP_HPP__
#define MSTL_CORE_CONTAINER_UNORDERED_MAP_HPP__
 ...
 *
 * 此文件提供了无序映射容器的实现。
 * unordered_map是一种基于哈希表的关联容器，包含具有唯一键的键值对。
 * 元素无序存储，支持平均常数时间复杂度的查找、插入和删除操作。
 * 底层使用哈希表实现。
 */

#include "MSTL/core/container/hashtable.hpp"
 ...
 * @tparam Alloc 分配器类型
 *
 * unordered_map是一种关联容器，存储键值对（key-value pairs），每个键在容器中唯一。
 * 元素无序存储，由哈希函数将键映射到桶中。
 * 支持快速的键查找。底层使用哈希表实现。
 */
...
public:
     ...
     * @return 最大桶数量
     */
    MSTL_NODISCARD size_type buckets_max_count() const noexcept {
     ...
     * @return 桶中的元素数量
     */
    MSTL_NODISCARD size_type bucket_size(size_type n) const noexcept {
    ...
    pair<iterator, bool> insert(value_type&& value) {
        return ht_.insert_unique(_MSTL forward<value_type>(value));
    }
     ...
     * @param key 要访问的键
     * @return 对应键的值的常量引用
     * @throw iterator_exception 如果键不存在
     */
...
#endif // MSTL_CORE_CONTAINER_UNORDERED_MAP_HPP__
