...
private:
    ...
    insert_unique_aux(Iterator first, Iterator last) {
        ...
        const size_type need_buckets = static_cast<size_type>(
            _MSTL ceil(static_cast<double>(size_ + n) / max_load_factor()));
        rehash(need_buckets);
    ...
    bool equal_small(const hashtable& rhs) const {
        for (const_iterator iter = begin(); iter != end(); ++iter) {
            ...
            const size_t count_lhs = _MSTL count_if(begin(), end(),
                [this, &key](const value_type& val) {
                    return equals_(extracter_(val), key);
                });
            const size_t count_rhs = _MSTL count_if(rhs.begin(), rhs.end(),
    ...
    bool equal_large(const hashtable& rhs) const {
        ...
        _MSTL sort(elements_lhs.begin(), elements_lhs.end());
        _MSTL sort(elements_rhs.begin(), elements_rhs.end());
...
public:
    ...
    : buckets_(_MSTL move(other.buckets_)),
      size_(other.size_),
      hasher_(_MSTL move(other.hasher_)),
      equals_(_MSTL move(other.equals_)),
      extracter_(_MSTL move(other.extracter_)),
      pair_(_MSTL move(other.pair_)){
        other.size_ = 0;
    }
     ...
     * @return 桶数量
     */
    MSTL_NODISCARD size_type buckets_size() const noexcept {
     ...
     * @return 最大桶数量
     */
    MSTL_NODISCARD static size_type buckets_max_size() noexcept {
     ...
     * @return 桶索引
     */
    MSTL_NODISCARD size_type bucket_index(const key_type& key) const
    noexcept(is_nothrow_hashable_v<key_type>) {
     ...
     * @return 桶中的元素数量
     */
    MSTL_NODISCARD size_type bucket_size(size_type index) const noexcept {
        size_type result = 0;
    ...
    void max_load_factor(const float lf) noexcept {
        MSTL_DEBUG_VERIFY(lf > 0, "hashtable load factor invalid.");
        pair_.value = lf;
    }
    ...
    void reserve(const size_type n) {
        ...
        const size_type needed = static_cast<size_type>(
            _MSTL ceil(static_cast<double>(n) / max_load_factor()));
    ...
    pair<iterator, bool> emplace_unique(Args&&... args) {
        ...
        const link_type node = hashtable::new_node(_MSTL forward<Args>(args)...);
    ...
    pair<iterator, bool> insert_unique(value_type&& value) {
        return hashtable::emplace_unique(_MSTL move(value));
    }
    ...
    iterator insert_equal(value_type&& value) {
        return hashtable::emplace_equal(_MSTL move(value));
    }
    ...
    noexcept(is_nothrow_swappable_v<HashFcn> &&
        ...
        _MSTL swap(hasher_, other.hasher_);
        _MSTL swap(equals_, other.equals_);
        _MSTL swap(extracter_, other.extracter_);
        buckets_.swap(other.buckets_);
        _MSTL swap(size_, other.size_);
