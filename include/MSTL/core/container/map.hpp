#ifndef MSTL_CORE_CONTAINER_MAP_HPP__
#define MSTL_CORE_CONTAINER_MAP_HPP__
...
public:
	...
	pair<iterator, bool> insert(value_type&& value) {
		return tree_.emplace_unique(_MSTL move(value));
	}
	...
	iterator insert(iterator position, value_type&& value) {
		return tree_.insert_unique(position, _MSTL move(value));
	}
	 ...
	 */
	MSTL_NODISCARD mapped_type& operator [](const key_type& key) {
	 ...
	 * 如果键不存在，则插入一个默认构造的值并返回其引用。
	 */
	MSTL_NODISCARD mapped_type& operator [](key_type&& key) {
		iterator iter = tree_.lower_bound(key);
		if (iter == end() || key_comp()(key, iter->first)) {
			iter = tree_.emplace_unique_hint(iter, _MSTL move(key), initialize<T>());
		}
		return iter->second;
	}
	 ...
	 */
	MSTL_NODISCARD const mapped_type& at(const key_type& key) const {
	 ...
	 * @throw value_exception 如果键不存在
	 */
	MSTL_NODISCARD mapped_type& at(const key_type& key) {
		iterator iter = tree_.lower_bound(key);
		if (iter == end() && key_comp()(iter->first, key)) {
			throw_exception(value_exception("the value of this key does not exists."));
		}
...
#endif // MSTL_CORE_CONTAINER_MAP_HPP__
