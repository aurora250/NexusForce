...
public:
     ...
     * 基于当前参数计算理论可容纳元素数量。
     */
    MSTL_NODISCARD size_t capacity() const noexcept {
     ...
     * @return 位数
     */
    MSTL_NODISCARD size_t bit_size() const noexcept {
        return m_;
    }
     ...
     * @return k值
     */
    MSTL_NODISCARD size_t hash_count() const noexcept {
        return k_;
    }
     ...
     */
    MSTL_NODISCARD size_t approximate_count() const noexcept {
        size_t bits_set = 0;
        for (size_t i = 0; i < m_; ++i) {
            if (bits_[i]) ++bits_set;
        }
     ...
     * 根据位数组中1的比例估算当前的误报率。
     */
    MSTL_NODISCARD double false_positive_rate() const noexcept {
        size_t bits_set = 0;
        for (size_t i = 0; i < m_; ++i) {
            if (bits_[i]) ++bits_set;
        }
     ...
     */
    MSTL_NODISCARD bool contains(const T& key) const noexcept {
