...
private:
	...
	compressed_pair<allocator_type, pointer> pair_{ default_construct_tag{}, nullptr };  ///< 压缩存储的分配器和容量末尾指针
...
private:
	 ...
	 * @param value 填充值
	 */
	MSTL_CONSTEXPR20 void fill_initialize(size_type n, const T& value) {
		start_ = pair_.get_base().allocate(n);
		_MSTL uninitialized_fill_n(start_, n, value);
		finish_ = start_ + n;
		pair_.value = finish_;
	}
	...
	range_initialize(Iterator first, Iterator last) {
	 ...
	 * @brief 释放内存
	 */
	MSTL_CONSTEXPR20 void deallocate() noexcept {
		if (start_) {
	...
	template <typename Iterator>
	MSTL_CONSTEXPR20 enable_if_t<!is_ranges_fwd_iter_v<Iterator>>
	range_insert(iterator position, Iterator first, Iterator last) {
		while (first != last) {
	...
	template <typename Iterator>
	MSTL_CONSTEXPR20 enable_if_t<!is_ranges_fwd_iter_v<Iterator>>
	assign_aux(Iterator first, Iterator last) {
		clear();
		while (first != last) {
			vector::push_back(*first);
			++first;
	...
	assign_aux(Iterator first, Iterator last) {
		...
		if (n > capacity()) {
			...
			_MSTL copy(first, mid, start_);
			finish_ = _MSTL uninitialized_copy(mid, last, finish_);
		} else {
			_MSTL copy(first, last, start_);
...
public:
	 ...
	 * @param other 源向量
	 */
	MSTL_CONSTEXPR20 vector(const vector& other) {
	 ...
	 * @return 自身引用
	 */
	MSTL_CONSTEXPR20 vector& operator =(const vector& other) {
	 ...
	 * @param other 源向量
	 */
	MSTL_CONSTEXPR20 vector(vector&& other)
	noexcept(is_nothrow_swappable_v<compressed_pair<allocator_type, pointer>>) {
		vector::swap(other);
	}
	 ...
	 * @return 自身引用
	 */
	MSTL_CONSTEXPR20 vector& operator =(vector&& other)
	noexcept(
	 ...
	 * @param ilist 初始化列表
	 */
	MSTL_CONSTEXPR20 vector(std::initializer_list<T> ilist)
	 ...
	 * @return 自身引用
	 */
	MSTL_CONSTEXPR20 vector& operator =(std::initializer_list<T> ilist) {
		if (ilist.size() > capacity()) {
			...
			iterator i = _MSTL copy(ilist.begin(), ilist.end(), begin());
			_MSTL destroy(i.base(), finish_);
		} else {
			_MSTL copy(ilist.begin(), ilist.begin() + size(), start_);
			_MSTL uninitialized_copy(ilist.begin() + size(), ilist.end(), finish_);
		}
		finish_ = start_ + ilist.size();
		return *this;
	}
	 ...
	 */
	MSTL_CONSTEXPR20 void resize(size_type new_size, const T& value) {
		if (new_size < size()) {
	...
	MSTL_CONSTEXPR20 void emplace(iterator position, Args&&... args) {
		if (finish_ != pair_.value) {
			if (position == end()) {
				...
				_MSTL move_backward(pos_ptr, finish_ - 2, finish_ - 1);
				_MSTL construct(pos_ptr, _MSTL forward<Args>(args)...);
			}
			return;
		}
		...
		try {
			new_finish = _MSTL uninitialized_move(begin(), position, new_start);
	...
	MSTL_CONSTEXPR20 void emplace_back(Args&&... args) {
		if (finish_ != pair_.value) {
			...
			vector::emplace(end(), _MSTL forward<Args>(args)...);
		}
	}
	 ...
	 * @param value 要插入的值
	 */
	MSTL_CONSTEXPR20 void push_back(const T& value) {
	 ...
	 * @param value 要插入的值
	 */
	MSTL_CONSTEXPR20 void push_back(T&& value) {
		vector::emplace_back(_MSTL move(value));
	}
	 ...
	 * @return 被移除的元素
	 */
	MSTL_CONSTEXPR20 T pop_back_v()
	noexcept(is_nothrow_destructible_v<T> && is_nothrow_move_constructible_v<T>) {
		MSTL_DEBUG_VERIFY(!empty(), "pop called in an empty vector")
		T value{_MSTL move(finish_ - 1)};
		_MSTL destroy(finish_ - 1);
		--finish_;
	...
	MSTL_CONSTEXPR20 void assign(size_type n, const value_type& value) {
		if (n > capacity()) {
			...
			try {
				new_finish = _MSTL uninitialized_fill_n(new_start, n, value);
			} catch (...) {
				_MSTL destroy(new_start, new_finish);
	 ...
	 * @param ilist 初始化列表
	 */
	MSTL_CONSTEXPR20 void assign(std::initializer_list<value_type> ilist) {
	 ...
	 * @return 指向插入元素的迭代器
	 */
	MSTL_CONSTEXPR20 iterator insert(iterator position, const value_type& value) {
	...
	MSTL_CONSTEXPR20 iterator insert(iterator position, value_type&& value) {
		size_type n = position - begin();
		vector::emplace(position, _MSTL move(value));
		return begin() + n;
	}
	 ...
	 * @param ilist 初始化列表
	 */
	MSTL_CONSTEXPR20 void insert(iterator position, std::initializer_list<value_type> ilist) {
	 ...
	 * @param value 要插入的值
	 */
	MSTL_CONSTEXPR20 void insert(iterator position, size_type n, const value_type& value) {
		if (n == 0) return;
		...
		if (static_cast<size_type>(pair_.value - finish_) >= n) {
			...
			if (elems_after > n) {
				...
				_MSTL uninitialized_fill_n(finish_, n - elems_after, value);
				finish_ += n - elems_after;
				_MSTL uninitialized_move(position, old_finish, end());
				finish_ += elems_after;
				_MSTL destroy(position, old_finish);
				_MSTL uninitialized_fill(position, old_finish, value);
			}
		} else {
			const size_type old_size = size();
			const size_type len = old_size + _MSTL max(old_size, n);
			...
			new_finish = _MSTL uninitialized_fill_n(new_finish, n, value);
			new_finish = _MSTL uninitialized_copy(position, end(), new_finish);
	...
	noexcept(is_nothrow_move_assignable_v<value_type> && is_nothrow_destructible_v<value_type>) {
		MSTL_DEBUG_VERIFY(_MSTL distance(first, last) >= 0, "vector erase out of ranges.");

	    const auto elems_after = end() - last;
	    if (elems_after > 0) {
	        _MSTL move_backward(last, end(), first + elems_after);
	    }
	...
	noexcept(is_nothrow_move_assignable_v<value_type>) {
		if (position + 1 != end()) {
		    _MSTL move(position + 1, end(), position);
		}
	...
	MSTL_NODISCARD MSTL_CONSTEXPR20 const_reference at(const size_type position) const noexcept {
		MSTL_DEBUG_VERIFY(position < size(), "vector access out of range");
	 ...
	 */
	MSTL_CONSTEXPR20 void swap(vector& other)
