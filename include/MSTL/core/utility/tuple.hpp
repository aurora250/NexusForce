...
struct tuple<> : icommon<tuple<>> {
	...
	MSTL_CONSTEXPR14 tuple& operator =(const tuple&) noexcept = default;  ///< 拷贝赋值运算符
...
private:
	...
	static constexpr size_t __broaden_tuple(const Tuple& tup, _MSTL index_sequence<Idx...> idx) noexcept;
 ...
 */
template <typename T, typename Tuple, size_t... Index>
MSTL_NODISCARD constexpr T __broaden_make_from_tuple(Tuple&& tup, index_sequence<Index...> idx)
...
MSTL_NODISCARD constexpr T make_from_tuple(Tuple&& tup)
noexcept(noexcept(_INNER __broaden_make_from_tuple<T>(
		_MSTL forward<Tuple>(tup),
 ...
 */
template <typename... Tuples>
MSTL_NODISCARD constexpr typename _INNER tuple_cat_bind_t<Tuples...>::Ret tuple_cat(Tuples&&... tuples) {
	...
	return _INNER __tuple_cat_in_turn<Ret>(
		ElementIdxSeq{},
		TupleIdxSeq{},
		_MSTL forward_as_tuple(_MSTL forward<Tuples>(tuples)...)
	);
}
