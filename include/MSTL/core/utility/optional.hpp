...
public:
    ...
    template <typename U = T, enable_if_t<!is_same_v<remove_cvref_t<U>, optional>
        ...
        && !convertible_from_optional<U>::value && !assignable_from_optional<U>::value, int> = 0>
    MSTL_CONSTEXPR20 optional& operator =(optional<U>&& other)
    noexcept(is_nothrow_constructible_v<T, U> && is_nothrow_assignable_v<T&, U>) {
        if (_MSTL addressof(other) == this) return *this;
        if (other) {
            if (have_value_) {
     ...
     * @param other 要交换的可选值
     */
    MSTL_CONSTEXPR20 void swap(optional& other)
    noexcept(is_nothrow_move_constructible_v<T> && is_nothrow_swappable_v<T>) {
        ...
        if (have_value_ && other.have_value_) {
            _MSTL swap(*this, other);
        } else if (have_value_) {
            other.emplace(_MSTL move(**this));
            reset();
        } else if (other.have_value_) {
            optional::emplace(_MSTL move(*other));
            other.reset();
        }
    }
};
...
public:
    ...
    template <typename F>
    constexpr decltype(auto) and_then(F&& f) const && {
        if (ptr_) return _MSTL forward<F>(f)(_MSTL move(*ptr_));
        return remove_cvref_t<decltype(f(*ptr_))>{};
    }
