...
/**
 * @defgroup VariantTypeTraits 变体类型特性
 * @brief 变体相关的类型特性
 * @{
 */
 ...
 */
template <typename Variant, typename T>
MSTL_INLINE17 constexpr size_t variant_index_v = variant_index<Variant, T>::value;
#endif
...
private:
     ...
     * 大小为所有类型中最大的大小。
     */
    alignas(_MSTL max({ alignof(Types)... })) byte_t union_[_MSTL max({ sizeof(Types)... })]{};
    ...
    static move_construct_function* move_constructors_table() noexcept {
        static move_construct_function function_ptrs[sizeof...(Types)] = {
            [](byte_t* union_dst, const byte_t* union_src) noexcept {
                new (union_dst) Types(_MSTL move(*reinterpret_cast<const Types*>(union_src)));
            }...
        };
        return function_ptrs;
    }
    ...
    using const_visitor_function = common_type_t<
        _MSTL invoke_result_t<Lambda, Types const&>...>(*)(byte_t const*, Lambda&&);  ///< 常量访问者函数指针类型
    ...
    static const_visitor_function<Lambda>* const_visitors_table() noexcept {
        static const_visitor_function<Lambda> function_ptrs[sizeof...(Types)] = {
            [](byte_t const* union_p, Lambda&& lambda) -> _MSTL invoke_result_t<Lambda, Types const&> {
    ...
    using visitor_function = common_type_t<
        _MSTL invoke_result_t<Lambda, Types&>...>(*)(byte_t*, Lambda&&);  ///< 访问者函数指针类型
    ...
    static visitor_function<Lambda>* visitors_table() noexcept {
        static visitor_function<Lambda> function_ptrs[sizeof...(Types)] = {
            [](byte_t* union_p, Lambda&& lambda) -> common_type_t<_MSTL invoke_result_t<Lambda, Types&>...> {
...
public:
    ...
    variant(Args&&... args) {
        if (!variant::try_construct(_MSTL forward<Args>(args)...)) {
            index_ = 0;
            new (union_) variant_alternative_t<variant, 0>();
        }
    }
    ...
    noexcept(is_nothrow_constructible_v<T, Args...>) {
        variant::emplace<variant_index_v<variant, T>>(_MSTL forward<Args>(args)...);
    }
    ...
    MSTL_CONSTEXPR20 void swap(variant& x) noexcept {
        ...
        alignas(_MSTL max({ alignof(Types)... })) byte_t temp_union[_MSTL max({ sizeof(Types)... })];
        ...
        move_constructors_table()[this_index](x.union_, reinterpret_cast<byte_t*>(temp_union));
