...
const T* __any_cast_aux_dispatch_impl(const _MSTL any* value, any_cast_true_tag) noexcept;
...
class MSTL_API any {
    ...
    using manage_t = conditional_t<
        ...
        sizeof(T) <= sizeof(storage_internal) &&
...
public:
    ...
    any& operator =(T&& value) {
        *this = any(_MSTL forward<T>(value));
        return *this;
    }
    ...
    const DT& emplace(Args&&... args) {
        any::try_emplace<DT>(_MSTL forward<Args>(args)...);
    ...
    const DT& emplace(std::initializer_list<U> ilist, Args&&... args) {
        any::try_emplace<DT, U>(ilist, _MSTL forward<Args>(args)...);
        return *manage_t<DT>::access(storage_);
    }
