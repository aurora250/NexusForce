...
private:
    ...
    template <typename T>
    static MSTL_CONSTEXPR20 bool check_empty(const T&) noexcept {
        return false;
    }
    static MSTL_CONSTEXPR20 bool check_empty(const char* value) noexcept {
        return !value || value[0] == 0;
    }

    template <typename T>
    MSTL_CONSTEXPR20 void out(const T& value) {
        if (this->check_empty(value)) return;
        if (!this->is_compact_) str_ += " ";
        this->str_ += to_string(value);
        this->is_compact_ = false;
    }

public:
    MSTL_CONSTEXPR20 output(string& str) noexcept
    : is_compact_(true), str_(str) {}

    MSTL_CONSTEXPR20 output& operator ()() noexcept {
        return *this;
    }

    MSTL_CONSTEXPR20 output& compact() noexcept {
        this->is_compact_ = true;
        return *this;
    }

    template <typename T1, typename... T>
    MSTL_CONSTEXPR20 output& operator ()(const T1& value, const T&... args) {
        this->out(value);
...
struct bracket {
    ...
    MSTL_CONSTEXPR20 bracket(output& out, const char* = nullptr)
    : out_(out) {
        out_("(").compact();
    }
...
struct bracket<false> {
    MSTL_CONSTEXPR20 bracket(output& out, const char* str = nullptr) {
        out(str);
    }
};
...
private:
    template <size_t NN>
    MSTL_CONSTEXPR20 enable_if_t<NN == 0> __bound_dispatch() const {
        out_("[]");
        return;
    }
...
public:
    ...
    MSTL_CONSTEXPR20 bound(output& out)
    : out_(out) {}
...
struct at_destruct {
    ...
    MSTL_CONSTEXPR20 at_destruct(output& out, const char* str = nullptr) noexcept
    : out_(out), str_(str) {}

    MSTL_CONSTEXPR20 ~at_destruct() {
        out_(str_);
    }

    MSTL_CONSTEXPR20 void set_str(const char* str = nullptr) noexcept {
        str_ = str;
    }
};
 ...
 * @return 反修饰后的可读名称
 */
string MSTL_API real_symbol_name(string name);
...
struct check<T[], IsBase> : check<T, true> {
    ...
    MSTL_CONSTEXPR20 check(const output& out)
    : base_t(out), bound_(out_), bracket_(out_) {}
};
...
struct parameter<IsStart, P1, P...> {
    ...
    MSTL_CONSTEXPR20 parameter(output& out) noexcept
    : out_(out) {}
