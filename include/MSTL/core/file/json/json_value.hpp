...
public:
     ...
     * @param value 字符串值
     */
    explicit json_string(string value) noexcept : value_(_MSTL move(value)) {}
...
public:
     ...
     * @return 成员映射表的常量引用
     */
    MSTL_NODISCARD const unordered_map<string, unique_ptr<json_value>>& get_members() const noexcept {
        return members_;
    }
};
...
public:
     ...
     * @return 元素列表的常量引用
     */
    MSTL_NODISCARD const vector<unique_ptr<json_value>>& get_elements() const noexcept { return elements_; }
};
 ...
 * @return 紧凑格式字符串
 */
MSTL_ALWAYS_INLINE_INLINE string to_string(const unique_ptr<json_value>& value) {
