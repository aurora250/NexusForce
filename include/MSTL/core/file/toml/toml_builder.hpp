...
private:
    ...
    toml_builder& value_impl(unique_ptr<T> value) {
        ...
        if (top.type == array) {
            top.array_ptr->add_element(_MSTL move(value));
            ...
            top.table_ptr->add_member(current_key_, _MSTL move(value));
            current_key_.clear();
        }
        return *this;
    }
...
public:
    ...
    toml_builder& value(string value) {
        return value_impl(make_unique<toml_string>(_MSTL move(value), toml_string::Basic));
    }
    ...
    toml_builder& value(unique_ptr<toml_value>&& value) {
        return value_impl(_MSTL move(value));
    }
    ...
    toml_builder& value_string(string value, toml_string::string_type type) {
        return value_impl(make_unique<toml_string>(_MSTL move(value), type));
    }
