...
public:
     ...
     * @return 存储的布尔值
     */
    MSTL_NODISCARD bool get_value() const noexcept { return value_; }
};
...
public:
     ...
     * @return 存储的64位整数值
     */
    MSTL_NODISCARD int64_t get_value() const noexcept { return value_; }
};
...
public:
     ...
     * @return 存储的双精度浮点数值
     */
    MSTL_NODISCARD double get_value() const noexcept { return value_; }
};
...
public:
    ...
    : value_(_MSTL move(value)), type_(type) {}
     ...
     * @return 引号类型枚举值
     */
    MSTL_NODISCARD string_type get_string_type() const noexcept { return type_; }
};
...
public:
     ...
     */
    MSTL_NODISCARD const datetime& get_value() const noexcept { return value_; }
     ...
     * @return 日期时间类型枚举值
     */
    MSTL_NODISCARD datetime_type get_datetime_type() const noexcept { return type_; }
};
...
public:
    ...
    void add_element(unique_ptr<toml_value> value) {
        elements_.emplace_back(_MSTL move(value));
    }
     ...
     * @return 元素个数
     */
    MSTL_NODISCARD size_t size() const noexcept { return elements_.size(); }

    /**
     * @brief 获取所有元素的常量引用
     * @return 元素列表的常量引用
     */
    MSTL_NODISCARD const vector<unique_ptr<toml_value>>& get_elements() const noexcept { return elements_; }
};
...
public:
     ...
     * @return 成员映射表的常量引用
     */
    MSTL_NODISCARD const unordered_map<string, unique_ptr<toml_value>>& get_members() const noexcept {
        return members_;
    }
     ...
     * @return 是否为内联表格
     */
    MSTL_NODISCARD bool is_inline() const noexcept { return is_inline_; }
...
/**
 * @brief toml值智能指针转换为紧凑格式字符串
 * @param value toml值智能指针
 * @return 紧凑格式字符串
 */
MSTL_ALWAYS_INLINE_INLINE string to_string(const unique_ptr<toml_value>& value) {
...
/**
 * @brief toml值智能指针转换为文档格式字符串
 * @param value toml值智能指针
 * @return 完整的toml文档字符串
 */
MSTL_ALWAYS_INLINE_INLINE string toml_document(const unique_ptr<toml_value>& value) {
