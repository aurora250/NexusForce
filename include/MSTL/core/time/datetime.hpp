...
public:
     ...
     * @return 年份
     */
    MSTL_NODISCARD constexpr date_type year() const noexcept {
        return year_;
    }
     ...
     * @return 月份
     */
    MSTL_NODISCARD constexpr date_type month() const noexcept {
        return month_;
    }
     ...
     * @return 日期
     */
    MSTL_NODISCARD constexpr date_type day() const noexcept {
        return day_;
    }
     ...
     */
    MSTL_NODISCARD constexpr static date parse(const string_view view) {
    ...
    constexpr void swap(date& other) noexcept {
        _MSTL swap(year_, other.year_);
        _MSTL swap(month_, other.month_);
        _MSTL swap(day_, other.day_);
    }
};
...
public:
     ...
     * @return 小时
     */
    MSTL_NODISCARD constexpr time_type hours() const noexcept {
        return hours_;
    }
     ...
     * @return 分钟
     */
    MSTL_NODISCARD constexpr time_type minutes() const noexcept {
        return minutes_;
    }
     ...
     * @return 秒
     */
    MSTL_NODISCARD constexpr time_type seconds() const noexcept {
        return seconds_;
    }
     ...
     * @throws value_exception 格式错误时抛出
     */
    MSTL_NODISCARD static constexpr time parse(const string_view view) {
...
public:
    using date_type = _MSTL date::date_type;  ///< 日期分量类型
    using time_type = _MSTL time::time_type;  ///< 时间分量类型

private:
    _MSTL date date_{};           ///< 日期部分
    _MSTL time time_{};           ///< 时间部分
...
public:
     ...
     * @param time 时间对象
     */
    constexpr explicit datetime(const _MSTL date& date, const _MSTL time& time) noexcept
     ...
     * @param offset 时区偏移（秒）
     */
    constexpr explicit datetime(const _MSTL date& date, const _MSTL time& time, const int64_t offset) noexcept
     ...
     * @param date 日期对象
     */
    constexpr explicit datetime(const _MSTL date& date) noexcept
    : date_(date) {}
     ...
     * @param time 时间对象
     */
    constexpr explicit datetime(const _MSTL time& time) noexcept
    : time_(time) {}
     ...
     * @param date 日期对象
     */
    constexpr explicit datetime(_MSTL date&& date) noexcept
    : date_(date) {
        date.clear();
    }
     ...
     * @param time 时间对象
     */
    constexpr explicit datetime(_MSTL time&& time) noexcept
    : time_(time) {
        time.clear();
    }
     ...
     * @param time 时间对象
     */
    constexpr explicit datetime(_MSTL date&& date, _MSTL time&& time) noexcept
    : date_(date), time_(time) {
        date.clear();
        time.clear();
    }
     ...
     * @param offset 时区偏移（秒）
     */
    constexpr explicit datetime(_MSTL date&& date, _MSTL time&& time, const int64_t offset) noexcept
    : date_(date), time_(time), offset_seconds_(offset), has_timezone_(true) {
        date.clear();
        time.clear();
    }
     ...
     * @return 自身引用
     */
    constexpr datetime& operator =(const _MSTL date& date) noexcept {
        date_ = date;
        return *this;
    }
     ...
     * @return 自身引用
     */
    constexpr datetime& operator =(_MSTL date&& date) noexcept {
        date_ = date;
        date.clear();
        return *this;
    }
     ...
     * @return 自身引用
     */
    constexpr datetime& operator =(const _MSTL time& time) noexcept {
        time_ = time;
        return *this;
    }
     ...
     * @return 自身引用
     */
    constexpr datetime& operator =(_MSTL time&& time) noexcept {
        time_ = time;
        time.clear();
        return *this;
    }
     ...
     * @return 日期对象
     */
    MSTL_NODISCARD constexpr const _MSTL date& date() const noexcept {
        return date_;
    }
     ...
     * @return 时间对象
     */
    MSTL_NODISCARD constexpr const _MSTL time& time() const noexcept {
        return time_;
    }
     ...
     * @return 是否有时区
     */
    MSTL_NODISCARD constexpr bool has_timezone() const noexcept {
     ...
     * @return 时区偏移（秒）
     */
    MSTL_NODISCARD constexpr int64_t offset_seconds() const noexcept {
     ...
     * @return 纪元起始时间
     */
    MSTL_NODISCARD static constexpr datetime epoch() noexcept {
        return datetime{};
    }
     ...
     * @throws value_exception 格式错误时抛出
     */
    MSTL_NODISCARD static constexpr datetime parse_ISO_UTC(const string_view view) {
        if (view.size() < 20 || view[10] != 'T') {
            throw_exception(value_exception("Invalid ISO UTC datetime format."));
        }

        const _MSTL date d = date::parse(view.substr(0, 10));
        const _MSTL time t = time::parse(view.substr(11, 8));
     ...
     * @return 是否解析成功
     */
    MSTL_CONSTEXPR20 bool try_parse_ISO_UTC(const string_view view) noexcept {
        try {
     ...
     * @throws value_exception 格式错误时抛出
     */
    MSTL_NODISCARD static constexpr datetime parse_GMT(string_view view) {
        if (view.size() < 29) {
     ...
     * @return 是否解析成功
     */
    MSTL_CONSTEXPR20 bool try_parse_GMT(const string_view view) noexcept {
        try {
     ...
     * @throws value_exception 格式错误时抛出
     */
    MSTL_NODISCARD static constexpr datetime parse_ISO(const string_view view) {
        if (view.size() < 19 || view[10] != 'T') {
            throw_exception(value_exception("Invalid ISO datetime format."));
        }
        const _MSTL date d = _MSTL date::parse(view.substr(0, 10));
        size_t time_len = 8;
        if (view.size() >= 19) {
            time_len = 8;
        }
        const _MSTL time t = _MSTL time::parse(view.substr(11, time_len));
        return datetime(d, t);
    }
     ...
     * @return 是否解析成功
     */
    MSTL_CONSTEXPR20 bool try_parse_ISO(const string_view view) noexcept {
        try {
     ...
     * @throws value_exception 格式错误时抛出
     */
    MSTL_NODISCARD static constexpr datetime parse(const string_view view) {
...
public:
     ...
     * @return 解析得到的时间戳
     */
    MSTL_NODISCARD static constexpr timestamp parse(const string_view view) {
