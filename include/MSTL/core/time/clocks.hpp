...
#include "MSTL/core/time/time_point.hpp"
...
struct MSTL_API system_clock {
    using duration = _MSTL nanoseconds;   ///< 持续时间类型
    using rep = duration::rep;            ///< 数值类型
    using period = duration::period;      ///< 时间单位比例
    using time_point = _MSTL time_point<system_clock>; ///< 时间点类型
    ...
    static time_point from_seconds(const seconds time_value) noexcept {
        using TP = _MSTL time_point<system_clock, seconds>;
...
struct MSTL_API steady_clock {
    ...
    using rep = duration::rep;            ///< 数值类型
    using period = duration::period;      ///< 时间单位比例
    using time_point = _MSTL time_point<steady_clock>; ///< 时间点类型
    ...
    to_system(const _MSTL time_point<steady_clock, Dur>& tp) {
