...
#include "MSTL/core/memory/allocated_ptr.hpp"
...
struct __future_base {
    ...
    public:
         ...
         */
        void set(Res&& result) {
            new (storage.addr()) Res(_MSTL forward<Res>(result));
            initialized = true;
        }
    };
    ...
    public:
        ...
        void set_result(Callable&& result_func, const bool ignore_failure = false) {
            ...
            call_once(flag, &state_base::do_set, this, _MSTL addressof(func), _MSTL addressof(did_set));
            if (did_set) {
    ...
    private:
        ...
        struct make_ready final : at_thread_exit_elt {
             ...
             */
            void set() {
                _MSTL at_thread_exit_register(this, &make_ready::run);
            }
        };
    };
...
public:
     ...
     * @return 引用结果
     */
    MSTL_NODISCARD Res& get() noexcept {
        return *value_ptr;
    }
};
