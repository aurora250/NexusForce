#ifndef MSTL_CORE_ASYNC_GENERATOR_HPP__
#define MSTL_CORE_ASYNC_GENERATOR_HPP__
...
public:
    ...
    struct promise_type {
        ...
        void unhandled_exception() {
            exception = _MSTL current_exception();
        }
    };
    ...
    struct iterator {
        ...
        T& operator *() const noexcept {
            MSTL_CONSTEXPR_ASSERT(handle.promise().current_value.has_value());
...
public:
    ...
    iterator begin() {
        if (handle_) {
            ...
            if (handle_.promise().exception) {
                _MSTL rethrow_exception(handle_.promise().exception);
            }
            if (handle_.done()) {
    ...
    invoke_result_t<F, T> map(F&& func) {
        for (auto&& value : *this) {
            co_yield func(_MSTL forward<decltype(value)>(value));
        }
    }
    ...
    generator filter(Pred&& pred) {
        for (auto&& value : *this) {
            if (pred(value)) {
                co_yield _MSTL forward<decltype(value)>(value);
            }
        }
    }
    ...
    void for_each(F&& func) {
        for (auto&& value : *this) {
            func(_MSTL forward<decltype(value)>(value));
        }
    }
    ...
    Acc fold(Acc init, F&& func) {
        Acc result = _MSTL move(init);
        for (auto&& value : *this) {
            result = func(_MSTL move(result), _MSTL forward<decltype(value)>(value));
        }
        return result;
    }
};
    ...
    struct awaiter {
        ...
        T await_resume() {
            if (handle.promise().exception) {
                _MSTL rethrow_exception(handle.promise().exception);
            }
            return _MSTL move(*handle.promise().result);
        }
    };
...
public:
    ...
    T get() {
        ...
        return _MSTL move(*handle_.promise().result);
    }
...
auto when_all(Tasks&&... tasks) -> task<tuple<typename Tasks::promise_type::result_type...>> {
    ...
    co_await _INNER when_all_helper<0>(results, _MSTL forward<Tasks>(tasks)...);
    co_return results;
}
...
task<T> retry(Factory&& factory, const size_t max_attempts,
    ...
    static_assert(_MSTL is_invocable_r_v<task<T>, Factory>, "Factory must return task<T>");
    ...
    for (size_t attempt = 0; attempt < max_attempts; ++attempt) {
        try {
            ...
            last_exception = _MSTL current_exception();
...
#endif // MSTL_CORE_ASYNC_GENERATOR_HPP__
