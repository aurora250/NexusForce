...
struct waiter_pool_base {
	...
	bool waiter_waiting() const noexcept {
		...
		platform_wait_t res = ::_InterlockedExchangeAdd(
			const_cast<volatile platform_wait_t*>(&wait), 0);
#else
		platform_wait_t res;
...
public:
	 ...
	 */
	template <typename U>
	static enable_if_t<platform_wait_valid_v<U>, platform_wait_t*>
	waiter_wait_addr(const U* addr, platform_wait_t*) {
	...
	template <typename U>
	static enable_if_t<!platform_wait_valid_v<U>, platform_wait_t*>
	waiter_wait_addr(const U*, platform_wait_t* wait) {
		return wait;
	}
	...
	static bool waiter_do_spin_v(platform_wait_t* addr, const U& old,
		...
		auto const pred = [=] {
			return _MSTL memory_compare<U>(old, f()) != 0;
		};
		waiter_base::waiter_do_spin_v_impl(addr, old, value);
		return _MSTL atomic_spin(pred, spin);
	}
...
public:
	...
	void waiter_do_wait_v(T old, Func f) {
		...
		} while (_MSTL memory_compare<T>(old, f()) == 0);
	}
