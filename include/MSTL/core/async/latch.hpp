...
public:
    ...
    : counter_(expected) {
        MSTL_CONSTEXPR_ASSERT(expected >= 0);
    }
    ...
    MSTL_ALWAYS_INLINE void count_down(const platform_wait_t update = 1) {
        auto const old_value = _MSTL atomic_fetch_sub(&counter_, update, memory_order_release);
        if (old_value == update) {
    ...
    MSTL_ALWAYS_INLINE bool try_wait() const noexcept {
        return _MSTL atomic_load(&counter_, memory_order_acquire) == 0;
    }
