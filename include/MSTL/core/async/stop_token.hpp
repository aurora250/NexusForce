...
#include "MSTL/core/async/semaphore.hpp"
...
private:
    ...
    struct stop_state {
        ...
        bool stop_possible() noexcept {
            return value.load(memory_order_acquire) & ~locked_bit;
        }
        ...
        bool stop_requested() noexcept {
            return value.load(memory_order_acquire) & stop_requested_bit;
        }
        ...
        void add_owner() noexcept {
            owners.fetch_add(1, memory_order_relaxed);
        }
        ...
        void add_stop_source() noexcept {
            value.fetch_add(ssrc_counter_inc, memory_order_relaxed);
        }
        ...
        void remove_stop_source() noexcept {
            value.fetch_sub(ssrc_counter_inc, memory_order_release);
        }
        ...
        void unlock() noexcept {
            value.fetch_sub(locked_bit, memory_order_release);
        }
        ...
        bool try_lock(value_type& current_value,
            const memory_order failure_order = memory_order_acquire) noexcept {
        ...
        bool try_lock_and_stop(value_type& current_value) noexcept {
            return do_try_lock(
                current_value, stop_requested_bit,
        ...
        bool do_try_lock(value_type& current_value, value_type new_bits,
            ...
            return value.compare_exchange_weak(
                current_value, current_value | new_bits,
