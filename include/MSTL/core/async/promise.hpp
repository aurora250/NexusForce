...
public:
    ...
    void set_value(Res&& value) {
        state().set_result(state_type::create_setter(this, _MSTL forward<Res>(value)));
    }
    ...
    void set_value_at_thread_exit(Res&& value) {
        state().set_delayed_result(state_type::create_setter(this, _MSTL forward<Res>(value)), future_ptr);
    }
...
struct __future_base::task_setter {
    ...
    PtrT operator ()() const noexcept {
        try {
            ...
            (*result_ptr)->error_ptr = _MSTL current_exception();
        }
...
__future_base::task_state<Func, Alloc, Res(Args...)>::reset() {
    return _INNER create_task_state<Res(Args...)>(_MSTL move(impl.function_ptr), static_cast<Alloc&>(impl));
}
