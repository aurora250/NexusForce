...
class counting_semaphore {
     ...
     * 使用CAS操作确保线程安全。
     */
    MSTL_ALWAYS_INLINE bool do_try_acquire() noexcept {
        auto old_value = _MSTL atomic_load(&counter_, memory_order_acquire);
        if (old_value == 0) {
            return false;
        }
        return _MSTL atomic_cmpexch_strong(
...
public:
    ...
    : counter_(desired) {
        MSTL_CONSTEXPR_ASSERT(desired >= 0);
    }
    ...
    void release(const platform_wait_t update = 1) noexcept {
        if (0 < _MSTL atomic_fetch_add(&counter_, update, memory_order_release)) {
            return;
        }
    ...
    void acquire() noexcept {
        ...
        _MSTL atomic_wait_address(&counter_, pred);
    }
    ...
    bool try_acquire() noexcept {
        ...
        return _MSTL atomic_spin(pred, [] { return false; });
    }
