...
void zlib_compressor::stream_compressor::soft_reset() {
    ...
    const int result = ::deflateReset(static_cast<::z_stream*>(stream_));
    check_zlib_error(result);
    bytes_input_ = 0;
    bytes_output_ = 0;
}
...
byte_vector zlib_compressor::stream_compressor::compress(const cbyte_view& data, const bool finish) {
    ...
    if (!data.empty()) {
        ...
        stream->next_in = const_cast<byte_t*>(data.data());
    ...
    do {
        ...
        stream->avail_out = chunk_size;
        stream->next_out = output.data() + output.size() - chunk_size;
        ...
        bytes_output_ += have;
             ...
             (finish && result != Z_STREAM_END && (stream->avail_in > 0 || result != Z_BUF_ERROR)));
...
void zlib_compressor::stream_decompressor::soft_reset() {
    ...
    const int result = ::inflateReset(static_cast<::z_stream*>(stream_));
    check_zlib_error(result);
    bytes_input_ = 0;
    bytes_output_ = 0;
}
...
byte_vector zlib_compressor::stream_decompressor::decompress(const byte_view& data, const bool finish) {
    ...
    do {
        ...
        result = ::inflate(stream, finish ? Z_FINISH : Z_NO_FLUSH);
