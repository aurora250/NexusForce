...
pgsql_tb_result::pgsql_tb_result(void* result, const bool owns) :
result_(result),
owns_result_(owns) {
    if (result_ != nullptr) {
        row_count_ = ::PQntuples(static_cast<::PGresult*>(result_));
        column_count_ = ::PQnfields(static_cast<::PGresult*>(result_));
    }
}
...
bool pgsql_tb_result::is_null(const size_type index) const {
    ...
    return ::PQgetisnull(static_cast<::PGresult*>(result_), static_cast<int>(current_row_), static_cast<int>(index)) !=
           0;
}
...
const vector<string_view>& pgsql_tb_result::column_names() const {
    if (column_names_.empty() && result_ != nullptr) {
        ...
        for (size_type i = 0; i < column_count_; ++i) {
            column_names_.emplace_back(::PQfname(static_cast<::PGresult*>(result_), static_cast<int>(i)));
        }
    }
...
string_view pgsql_tb_result::get(const size_type index) const {
    ...
    return ::PQgetvalue(static_cast<::PGresult*>(result_), static_cast<int>(current_row_), static_cast<int>(index));
}
...
vector<char> pgsql_tb_result::get_blob(const size_type index) const {
    ...
    const string_view value =
            ::PQgetvalue(static_cast<::PGresult*>(result_), static_cast<int>(current_row_), static_cast<int>(index));
    const int length =
            ::PQgetlength(static_cast<::PGresult*>(result_), static_cast<int>(current_row_), static_cast<int>(index));
...
column_meta pgsql_tb_result::column_metadata(const size_type index) const {
    ...
    if (result_ != nullptr && index < column_count_) {
        meta.name = ::PQfname(static_cast<::PGresult*>(result_), static_cast<int>(index));
        meta.type = static_cast<int32_t>(::PQftype(static_cast<::PGresult*>(result_), static_cast<int>(index)));
        meta.max_length = static_cast<size_t>(::PQfsize(static_cast<::PGresult*>(result_), static_cast<int>(index)));
        meta.nullable = ::PQfnumber(static_cast<::PGresult*>(result_),
                                    ::PQfname(static_cast<::PGresult*>(result_), static_cast<int>(index))) != -1;
    }
    return meta;
}
