...
bool mysql_connect::connect(const db_config& config) {
    last_error_.clear();
    last_errno_ = 0;
    const ::MYSQL* p = ::mysql_real_connect(static_cast<::MYSQL*>(link_), config.host.data(), config.username.data(),
                                            config.password.data(), config.database.data(),
                                            static_cast<int>(config.port.value()), nullptr, 0);
    if (p == nullptr) {
        last_error_ = ::mysql_error(static_cast<::MYSQL*>(link_));
        last_errno_ = ::mysql_errno(static_cast<::MYSQL*>(link_));
        return false;
    }
...
bool mysql_connect::reconnect(const db_config& config) {
    if (connected()) {
        ::mysql_close(static_cast<::MYSQL*>(link_));
        ensure_mysql_thread_cleanup();
        link_ = ::mysql_init(nullptr);
...
bool mysql_connect::set_character_set(const string& encoding) noexcept {
    return connected() && ::mysql_set_character_set(static_cast<::MYSQL*>(link_), encoding.data()) == 0;
}

bool mysql_connect::set_options(const mysql_option option, const string& str) const noexcept {
    return connected() &&
           ::mysql_options(static_cast<::MYSQL*>(link_), static_cast<::mysql_option>(option), str.data()) == 0;
}

string_view mysql_connect::get_character_set() const noexcept {
    return ::mysql_character_set_name(static_cast<::MYSQL*>(link_));
}
...
bool mysql_connect::update(const string& sql) const {
    if (::mysql_query(static_cast<::MYSQL*>(link_), sql.data()) != 0) {
        last_error_ = ::mysql_error(static_cast<::MYSQL*>(link_));
        last_errno_ = ::mysql_errno(static_cast<::MYSQL*>(link_));
        return false;
    }
...
unique_ptr<idb_tb_result> mysql_connect::query(const string& sql) const {
    ...
    return make_unique<mysql_result>(::mysql_store_result(static_cast<::MYSQL*>(link_)));
}
...
bool mysql_connect::is_valid() const noexcept { return ::mysql_ping(static_cast<::MYSQL*>(link_)) == 0; }
...
idb_result* mysql_factory::create_result(void* native_result) {
    // NOLINTNEXTLINE(cppcoreguidelines-owning-memory)
    return new mysql_result(native_result);
}
