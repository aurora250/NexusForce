...
namespace {
    ...
    constexpr uint32_t md5_rotleft(const uint32_t x, const uint32_t c) {
        return (x << c) | (x >> (32 - c));
    }
    constexpr uint32_t md5_F(const uint32_t x, const uint32_t y, const uint32_t z) {
        return (x & y) | (~x & z);
    }
    constexpr uint32_t md5_G(const uint32_t x, const uint32_t y, const uint32_t z) {
        return (x & z) | (y & ~z);
    }
    constexpr uint32_t md5_H(const uint32_t x, const uint32_t y, const uint32_t z) {
        return x ^ y ^ z;
    }
    constexpr uint32_t md5_I(const uint32_t x, const uint32_t y, const uint32_t z) {
        return y ^ (x | ~z);
    }


    constexpr uint32_t SHA1_rotleft(const uint32_t x, const uint32_t c) {
        return (x << c) | (x >> (32 - c));
    }
    ...
    constexpr uint32_t SHA256_rotr(const uint32_t x, const uint32_t n) {
        return (x >> n) | (x << (32 - n));
    }
    constexpr uint32_t SHA256_ch(const uint32_t x, const uint32_t y, const uint32_t z) {
        return (x & y) ^ (~x & z);
    }
    ...
    constexpr uint32_t SHA256_gamma0(const uint32_t x) {
        return SHA256_rotr(x, 7) ^ SHA256_rotr(x, 18) ^ (x >> 3);
    }
    constexpr uint32_t SHA256_gamma1(const uint32_t x) {
    ...
    constexpr byte_t AES256_rcon[15] = {
    ...
    constexpr void inv_mix_columns(byte_t state[16]) {
        for (int c = 0; c < 4; ++c) {
            ...
            state[c * 4] =
                AES256_gf_mult(0x0e, s0) ^ AES256_gf_mult(0x0b, s1) ^
                AES256_gf_mult(0x0d, s2) ^ AES256_gf_mult(0x09, s3);
            state[c * 4 + 1] =
                AES256_gf_mult(0x09, s0) ^ AES256_gf_mult(0x0e, s1) ^
                AES256_gf_mult(0x0b, s2) ^ AES256_gf_mult(0x0d, s3);
            state[c * 4 + 2] =
                AES256_gf_mult(0x0d, s0) ^ AES256_gf_mult(0x09, s1) ^
                AES256_gf_mult(0x0e, s2) ^ AES256_gf_mult(0x0b, s3);
            state[c * 4 + 3] =
                AES256_gf_mult(0x0b, s0) ^ AES256_gf_mult(0x0d, s1) ^
                AES256_gf_mult(0x09, s2) ^ AES256_gf_mult(0x0e, s3);
        }
    }
