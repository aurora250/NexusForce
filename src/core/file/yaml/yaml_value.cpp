...
static void stringify_value(string& result, const yaml_value* value,
    size_t indent, bool inline_context);

static void stringify_sequence(string& result, const yaml_sequence* seq,
    const size_t indent, const bool inline_context) {
...
static void stringify_mapping(string& result, const yaml_mapping* map,
    const size_t indent, const bool inline_context) {
...
string yaml_value_to_string(const yaml_value* value) {
    string result;
    stringify_value(result, value, 0, true);
    return result;
}

string yaml_value_document(const yaml_value* root) {
    if (!root) return "";
    string result;

    if (root->is_sequence() || root->is_mapping()) {
        stringify_value(result, root, 0, false);
    } else {
        stringify_value(result, root, 0, true);
    }
