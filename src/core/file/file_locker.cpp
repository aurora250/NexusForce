...
bool file_locker::lock(const difference_type offset, const difference_type length, const file_lock mode) const noexcept {
    if (offset < 0 || length < 0) return false;

#ifdef NEFORCE_PLATFORM_WINDOWS
    ::OVERLAPPED ov{};
    const ::ULARGE_INTEGER off_ul = {
...
bool file_locker::try_lock(const difference_type offset, const difference_type length, const file_lock mode) const noexcept {
    return lock(offset, length, mode | file_lock::FAIL_IMMEDIATELY);
}

bool file_locker::is_locked(const difference_type offset, const difference_type length, file_lock* lock_out) const noexcept {
#ifdef NEFORCE_PLATFORM_WINDOWS
    ...
    if (::fcntl(handle_, F_GETLK, &fl) == -1) return false;
...
bool file_locker::lock_whole(const file_lock mode) const noexcept {
    return lock(0, 0, mode);
}

bool file_locker::unlock_whole() const noexcept {
    return unlock(0, 0);
}
...
: locker_(locker), offset_(offset), length_(length) {
