...
json_builder& json_builder::end_object() {
    if (contexts_.empty() || contexts_.top().type != object) {
        NEFORCE_THROW_EXCEPTION(json_exception("No object to close or context mismatch"));
    }
    if (!current_key_.empty()) {
        NEFORCE_THROW_EXCEPTION(json_exception("Incomplete key-value pair in object"));
    }
    contexts_.pop();
    return *this;
}
...
json_builder& json_builder::end_array() {
    if (contexts_.empty() || contexts_.top().type != array) {
        NEFORCE_THROW_EXCEPTION(json_exception("No array to close or context mismatch"));
    }
    contexts_.pop();
    return *this;
}
...
json_builder& json_builder::key(const string& key) {
    if (contexts_.empty() || contexts_.top().type != object) {
        NEFORCE_THROW_EXCEPTION(json_exception("Key can only be set inside an object"));
    }
    if (!current_key_.empty()) {
        NEFORCE_THROW_EXCEPTION(json_exception("Key already set without corresponding value"));
    }
    current_key_ = key;
    return *this;
}
...
unique_ptr<json_value> json_builder::build() {
    if (!contexts_.empty()) {
        NEFORCE_THROW_EXCEPTION(json_exception("Incomplete JSON structure - unclosed objects or arrays"));
    }
    if (!current_key_.empty()) {
        NEFORCE_THROW_EXCEPTION(json_exception("Incomplete key-value pair"));
    }
    if (!root_) {
