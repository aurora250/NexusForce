...
#    ifdef NEFORCE_COMPILER_MSVC
        ...
        const uint64_t t1_lo = ::_umul128(a.lo, b.hi, &t1_hi);
        const uint64_t t2_lo = ::_umul128(a.hi, b.lo, &t2_hi);
        const byte_t c1 = ::_addcarry_u64(0, result.hi, t1_lo, &result.hi);
        (void) ::_addcarry_u64(c1, result.hi, t2_lo, &result.hi);
        // 最终进位超出128位，截断丢弃
#    else
...
#    ifdef NEFORCE_COMPILER_MSVC
    const byte_t carry = ::_addcarry_u64(0, lo, other.lo, &lo);
    ::_addcarry_u64(carry, hi, other.hi, &hi);
#    else
...
uint128_t& uint128_t::operator-=(const uint128_t& other) {
#    ifdef NEFORCE_COMPILER_MSVC
    const byte_t borrow = ::_subborrow_u64(0, lo, other.lo, &lo);
    ::_subborrow_u64(borrow, hi, other.hi, &hi);
#    else
    const auto old_lo = this->lo;
    lo -= other.lo;
