...
void logger::stop_worker() {
    ...
    if (worker_.joinable()) {
        auto timeout = _MSTL chrono::seconds(5);
        if (worker_.joinable()) {
            auto start = _MSTL chrono::steady_clock::now();
            while (worker_.joinable() && _MSTL chrono::steady_clock::now() - start < timeout) {
                _MSTL this_thread::sleep_for(_MSTL chrono::milliseconds(10));
            }
...
void logger::worker_loop() {
    while (running_.load(_MSTL memory_order_acquire)) {
...

        {
            ...
            cv_.wait_for(lock, _MSTL chrono::milliseconds(100), [this] {
                return !queue_.empty() ||
